...
if command -v curl >/dev/null; then run curl -ksSf "${SIGOV_API_URL:-http://localhost:5001}/swagger/v1/swagger.json"; run curl -ksSf "${SIGOV_API_URL:-http://localhost:5001}/api/health/live"; run bash scripts/check-critical-pages.sh; fi
