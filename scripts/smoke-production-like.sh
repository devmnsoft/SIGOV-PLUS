...
out="${SIGOV_SMOKE_OUTPUT:-artifacts/smoke/rc50_54_prod_gate_result.txt}"
...
for required_tool in psql pg_dump pg_restore; do
  if ! command -v "$required_tool" >/dev/null; then
    log "FAIL tooling tool=$required_tool reason=not_found classification=P0_ENVIRONMENTAL"
    blocked=1
  fi
done
...
else log 'SKIP API probes reason=SIGOV_API_BASE_URL_not_set classification=OPTIONAL_RUNTIME_NOT_REQUESTED'; fi
...
else log 'SKIP Web probes reason=SIGOV_WEB_BASE_URL_not_set classification=OPTIONAL_RUNTIME_NOT_REQUESTED'; fi
