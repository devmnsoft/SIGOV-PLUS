param([string]$BaseUrl = $(if ($env:SIGOV_WEB_BASE_URL) {$env:SIGOV_WEB_BASE_URL} elseif ($env:SIGOV_WEB_URL) {$env:SIGOV_WEB_URL} else {'http://localhost:5000'}), [string]$ApiUrl = $(if ($env:SIGOV_API_BASE_URL) {$env:SIGOV_API_BASE_URL} elseif ($env:SIGOV_API_URL) {$env:SIGOV_API_URL} else {'http://localhost:5001'}), [string]$Output = 'artifacts/smoke/rc50_54_critical_pages_result.txt')
$ErrorActionPreference='Continue'; New-Item (Split-Path $Output) -ItemType Directory -Force | Out-Null; Set-Content $Output ''
$failed=$false
foreach($path in @('/api/health/live','/api/observabilidade/health','/api/observabilidade/liveness','/swagger/v1/swagger.json')) {
