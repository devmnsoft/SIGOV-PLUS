...
param(
    ...
    [string]$Database = $(if ($env:SIGOV_DB_NAME) { $env:SIGOV_DB_NAME } else { 'sigov' }),
    [string]$User = $(if ($env:SIGOV_DB_USER) { $env:SIGOV_DB_USER } else { 'sigov' }),
    [string]$Password = $(if ($env:SIGOV_DB_PASSWORD) { $env:SIGOV_DB_PASSWORD } else { $env:PGPASSWORD }),
    [string]$MaintenanceUser = $(if ($env:SIGOV_DB_ADMIN_USER) { $env:SIGOV_DB_ADMIN_USER } else { 'postgres' }),
    [string]$MaintenancePassword = $env:PGPASSWORD
)
...
if (-not $state.schema -or -not $state.usuario_table -or -not $state.admin_exists -or -not $state.admin_active -or -not $state.hash_accepted) {
