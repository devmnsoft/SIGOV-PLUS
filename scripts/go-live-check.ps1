param(
  [string]$PackagePath = 'artifacts/release/sigov-plus-1.0.0-rc20',
  [switch]$AllowWarnings,
  [switch]$StaticOnly,
  [string]$ReleaseCandidateVersion = '1.0.0-rc20'
)
...
$md=@('# Go-live check Pós-RC 20','',"Gerado em $($summary.generatedAt).",'',"Resumo: PASS=$($summary.passed) WARN=$($summary.warnings) FAIL_BLOCKING=$($summary.failedBlocking) STATUS=$($summary.statusFinal)",'','| Check | Status | Mensagem |','|---|---|---|')
