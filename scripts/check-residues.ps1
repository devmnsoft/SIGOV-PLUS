...
$patterns = @('SIGOV_PLUS','SIGOV\+','SIGOV-PLUS','SigovPlus','dbo\.','nvarchar','datetime2','uniqueidentifier','rowversion','schema core\.','schema sec\.','schema audit\.','schema lgpd\.','schema fin\.','schema trib\.','schema compras\.','schema rh\.','schema educacao\.','schema saude\.','schema saneamento\.','schema social\.','schema suporte\.','schema operacao\.','schema integracao\.','schema bi\.','schema transparencia\.','senha fixa','token em texto puro','secret em texto puro','connection string com senha versionada','stack trace para usuário final','projeto é uma POC','apenas demonstração')
$files = Get-ChildItem -Recurse -File | Where-Object { $_.FullName -notmatch '[/\\](\.git|bin|obj|tests)[/\\]' -and $_.FullName -notmatch 'check-residues\.ps1$' }
$findings = @()
