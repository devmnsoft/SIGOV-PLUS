...
-- Pode ser executado vÃ¡rias vezes. NÃ£o contÃ©m remoÃ§Ã£o destrutiva.
...
-- As seÃ§Ãµes abaixo consolidam as migrations idempotentes do repositÃ³rio: base, SaaS, tributÃ¡rio,
-- comercial/varejo/atacado/PDV/caixa, estoque/compras, OS, manutenÃ§Ã£o, mÃ³dulos pÃºblicos e indÃºstria.
...
-- Contexto mantido no cÃ³digo por bounded context; novas tabelas deste grupo devem ser criadas diretamente no schema sigov.
...
begin
    ...
    values ('Prefeitura Municipal de DemonstraÃ§Ã£o', '00000000000191', true, 'Registro de desenvolvimento sigov')
    ...
    values
        (v_entidade_id, v_exercicio_id, 'core', 'core_admin', 'PermissÃ£o administrativa do nÃºcleo', true),
        (v_entidade_id, v_exercicio_id, 'seguranca', 'seguranca_admin', 'PermissÃ£o administrativa de seguranÃ§a', true),
        (v_entidade_id, v_exercicio_id, 'auditoria', 'auditoria_admin', 'PermissÃ£o administrativa de auditoria', true),
        (v_entidade_id, v_exercicio_id, 'lgpd', 'lgpd_admin', 'PermissÃ£o administrativa LGPD', true),
        (v_entidade_id, v_exercicio_id, 'suporte', 'suporte_admin', 'PermissÃ£o administrativa de suporte', true),
        (v_entidade_id, v_exercicio_id, 'conformidade', 'aderencia_admin', 'PermissÃ£o administrativa de conformidade e aderÃªncia', true)
    on conflict do nothing;
...
begin
    ...
    foreach legacy_schema in array legacy_schemas loop
        if exists (select 1 from information_schema.schemata where schema_name = legacy_schema) then
            ...
            null; -- comando legado omitido por regra de seguranÃ§a sem remoÃ§Ã£o destrutiva.
            end if;
        end if;
    end loop;
end $$;
...
-- MÃ³dulo Processos Digitais, Protocolo, GED bÃ¡sico, Ouvidoria e DiÃ¡rio Oficial.
...
cross join (values
    ...
    ('LicitaÃ§Ã£o', 'Processos de contrataÃ§Ã£o pÃºblica', 60),
    ('Requerimento', 'Requerimentos de cidadÃ£os e empresas', 20),
...
from (values
 ...
 ('processos.ouvidoria.criar','Criar manifestaÃ§Ãµes de ouvidoria'),
 ('processos.ouvidoria.responder','Responder manifestaÃ§Ãµes de ouvidoria'),
 ('processos.ouvidoria.converter','Converter ouvidoria em processo'),
 ('processos.ouvidoria.arquivar','Arquivar ouvidoria'),
 ('processos.diario.visualizar','Visualizar diÃ¡rio oficial'),
 ('processos.diario.criar','Criar publicaÃ§Ãµes do diÃ¡rio oficial'),
 ('processos.diario.editar','Editar publicaÃ§Ãµes do diÃ¡rio oficial'),
 ('processos.diario.publicar','Publicar diÃ¡rio oficial')
...
values ('financeiro', 'Financeiro/SIAFIC', 'Base financeira e orÃ§amentÃ¡ria SIAFIC do sigov.', 'Operacional', 20, '/Financeiro', 'cash-coin', true)
...
from (values
 ...
 ('financeiro.orcamento.visualizar','Visualizar orÃ§amento'),('financeiro.orcamento.criar','Criar orÃ§amento'),('financeiro.orcamento.editar','Editar orÃ§amento'),('financeiro.orcamento.movimentar','Movimentar orÃ§amento'),
 ...
 ('financeiro.liquidacao.visualizar','Visualizar liquidaÃ§Ãµes'),('financeiro.liquidacao.criar','Criar liquidaÃ§Ãµes'),('financeiro.liquidacao.anular','Anular liquidaÃ§Ãµes'),
...
select t.id, e.id, ex.id, '1500', 'Recursos nÃ£o vinculados de impostos', 'Seed financeiro bÃ¡sico'
...
-- Etapa 2 - Pessoa e EndereÃ§o: reforÃ§os operacionais, Ã­ndices e permissÃµes granulares.
...
values
    ('core', 'pessoas', 'visualizar', 'core.pessoas.visualizar', 'Visualizar pessoas e endereÃ§os', true),
    ...
    ('core', 'enderecos', 'gerenciar', 'core.enderecos.gerenciar', 'Gerenciar endereÃ§os de pessoas', true),
    ('core', 'pessoas', 'exportar', 'core.exportar', 'Exportar cadastro de pessoas', true)
on conflict do nothing;
-- >>> database/postgres/migrations/020_rh_completo.sql
-- MÃ³dulo Recursos Humanos completo: cadastros, folha, ponto, fÃ©rias, afastamentos, saÃºde ocupacional,
-- eSocial estrutural, portal do servidor, outbox de eventos e integraÃ§Ã£o futura com Financeiro/SIAFIC.
...
values
    ...
    ('rh', 'financeiro', 'integrar', 'rh.financeiro.integrar', 'Preparar integraÃ§Ã£o de folha com Financeiro/SIAFIC', true)
on conflict do nothing;
-- >>> database/postgres/migrations/021_educacao_base.sql
-- EducaÃ§Ã£o Base: escolas, ano letivo, cursos, turmas, alunos, matrÃ­culas, frequÃªncia, avaliaÃ§Ãµes,
-- prÃ©-matrÃ­cula, Educacenso estrutural, portal educacional, dashboard, auditoria tÃ©cnica e outbox.
-- Todas as estruturas operacionais permanecem no schema Ãºnico sigov.
...
insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo)
values ('educacao', 'EducaÃ§Ã£o', 'MÃ³dulo base de gestÃ£o escolar do sigov', 'Operacional', 40, '/Educacao/Dashboard', 'book', true)
...
('educacao','matricula','visualizar','educacao.matricula.visualizar','Visualizar matrÃ­culas',true),
('educacao','matricula','criar','educacao.matricula.criar','Criar matrÃ­cula',true),
('educacao','matricula','cancelar','educacao.matricula.cancelar','Cancelar matrÃ­cula',true),
('educacao','matricula','transferir','educacao.matricula.transferir','Transferir matrÃ­cula',true),
...
('educacao','frequencia','visualizar','educacao.frequencia.visualizar','Visualizar frequÃªncia',true),
('educacao','frequencia','criar','educacao.frequencia.criar','Registrar frequÃªncia',true),
('educacao','avaliacao','visualizar','educacao.avaliacao.visualizar','Visualizar avaliaÃ§Ãµes',true),
('educacao','avaliacao','criar','educacao.avaliacao.criar','Criar avaliaÃ§Ã£o',true),
('educacao','nota','criar','educacao.nota.criar','Registrar nota',true),
('educacao','pre_matricula','visualizar','educacao.pre_matricula.visualizar','Visualizar prÃ©-matrÃ­cula',true),
('educacao','pre_matricula','criar','educacao.pre_matricula.criar','Criar prÃ©-matrÃ­cula',true),
('educacao','pre_matricula','converter','educacao.pre_matricula.converter','Converter prÃ©-matrÃ­cula',true),
...
('educacao','dashboard','visualizar','educacao.dashboard.visualizar','Visualizar dashboard educaÃ§Ã£o',true),
('educacao','exportar','exportar','educacao.exportar','Exportar dados de educaÃ§Ã£o',true)
on conflict do nothing;

-- Seeds de desenvolvimento seguros: sÃ³ inserem se existir tenant e entidade base.
do $$
declare
    v_tenant bigint;
    v_entidade bigint;
...
begin
    ...
    if v_tenant is not null and v_entidade is not null then
        ...
        values (v_tenant, v_entidade, v_curso, '1ANO', '1Âº Ano', 1)
        ...
        if v_escola is not null and v_ano is not null and v_curso is not null and v_serie is not null then
            ...
            values (v_tenant, v_entidade, v_exercicio, v_escola, v_ano, v_curso, v_serie, '1A', '1Âº Ano A', 'MATUTINO', 30, 'ABERTA')
            on conflict (tenant_id, entidade_id, ano_letivo_id, codigo) do nothing;
        end if;
    end if;
end $$;
-- >>> database/postgres/migrations/022_saude_acs_base.sql
-- SaÃºde/ACS base - schema Ãºnico sigov, PostgreSQL, Dapper-ready e idempotente.
create extension if not exists pgcrypto;
...
insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo)
values ('saude', 'SaÃºde', 'MÃ³dulo base de saÃºde, ACS, pacientes, atendimentos, farmÃ¡cia e vacinaÃ§Ã£o do sigov', 'Operacional', 50, '/Saude/Dashboard', 'heart-pulse', true)
on conflict (codigo) do nothing;

insert into sigov.feature_flag_def (codigo, nome, descricao, modulo, ativo)
values ('saude.acs_offline', 'ACS offline estrutural', 'Habilita endpoints e tela de sincronizaÃ§Ã£o offline estrutural do ACS.', 'saude', true)
on conflict (codigo) do nothing;
...
insert into sigov.permissao (modulo, recurso, acao, chave, descricao, ativo)
values
('saude','unidade','visualizar','saude.unidade.visualizar','Visualizar unidades de saÃºde',true),('saude','unidade','criar','saude.unidade.criar','Criar unidades de saÃºde',true),('saude','unidade','editar','saude.unidade.editar','Editar unidades de saÃºde',true),('saude','unidade','excluir','saude.unidade.excluir','Excluir unidades de saÃºde',true),
('saude','profissional','visualizar','saude.profissional.visualizar','Visualizar profissionais de saÃºde',true),('saude','profissional','criar','saude.profissional.criar','Criar profissionais de saÃºde',true),('saude','profissional','editar','saude.profissional.editar','Editar profissionais de saÃºde',true),('saude','profissional','excluir','saude.profissional.excluir','Excluir profissionais de saÃºde',true),
...
('saude','prontuario','visualizar','saude.prontuario.visualizar','Visualizar prontuÃ¡rio',true),('saude','prontuario','editar','saude.prontuario.editar','Editar prontuÃ¡rio',true),('saude','prontuario','visualizar_dados_sensiveis','saude.prontuario.visualizar_dados_sensiveis','Visualizar dados sensÃ­veis de prontuÃ¡rio',true),
...
('saude','farmacia','visualizar','saude.farmacia.visualizar','Visualizar farmÃ¡cia',true),('saude','farmacia','produto.criar','saude.farmacia.produto.criar','Criar produto de farmÃ¡cia',true),('saude','farmacia','dispensar','saude.farmacia.dispensar','Dispensar medicamento',true),
('saude','vacinacao','visualizar','saude.vacinacao.visualizar','Visualizar vacinaÃ§Ã£o',true),('saude','vacinacao','criar','saude.vacinacao.criar','Registrar vacinaÃ§Ã£o',true),
('saude','laboratorio','visualizar','saude.laboratorio.visualizar','Visualizar laboratÃ³rio',true),('saude','laboratorio','criar','saude.laboratorio.criar','Criar exame',true),('saude','laboratorio','resultado','saude.laboratorio.resultado','Registrar resultado de exame',true),
('saude','regulacao','visualizar','saude.regulacao.visualizar','Visualizar regulaÃ§Ã£o',true),('saude','regulacao','criar','saude.regulacao.criar','Criar regulaÃ§Ã£o',true),('saude','regulacao','editar','saude.regulacao.editar','Editar regulaÃ§Ã£o',true),
...
('saude','dashboard','visualizar','saude.dashboard.visualizar','Visualizar dashboard saÃºde',true),('saude','exportar','exportar','saude.exportar','Exportar dados de saÃºde',true)
on conflict do nothing;
...
begin
    ...
    if v_tenant is not null and v_entidade is not null then
        ...
        values (v_tenant, v_entidade, 'UBS-DEMO', 'Unidade BÃ¡sica de SaÃºde Demo', null, 'BASICA', 'ATIVA', '{}'::jsonb, '{}'::jsonb)
        on conflict (tenant_id, entidade_id, codigo) do nothing;
        ...
        values (v_tenant, v_entidade, 'MED-DEMO', 'Medicamento Demo', 'PrincÃ­pio ativo demo', 'UN')
        on conflict (tenant_id, entidade_id, codigo) do nothing;
        ...
        if v_unidade is not null and v_produto is not null then
            ...
            values (v_tenant, v_entidade, v_unidade, 'MA-DEMO', 'MicroÃ¡rea Demo')
            on conflict (tenant_id, entidade_id, codigo) do nothing;
        end if;
    end if;
end $$;
-- >>> database/postgres/migrations/023_saneamento_base.sql
-- Saneamento base - schema Ãºnico sigov, PostgreSQL, Dapper-ready e idempotente.
create extension if not exists pgcrypto;
...
insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo)
values ('saneamento','Saneamento','Consumidores, ligaÃ§Ãµes, leituras, faturas, laboratÃ³rio e rede estrutural.','Operacional',80,'/Saneamento/Dashboard','droplet',true)
on conflict (codigo) do nothing;
...
('saneamento','ligacao','visualizar','saneamento.ligacao.visualizar','Visualizar ligaÃ§Ãµes',true),
('saneamento','ligacao','criar','saneamento.ligacao.criar','Criar ligaÃ§Ãµes',true),
('saneamento','ligacao','editar','saneamento.ligacao.editar','Editar ligaÃ§Ãµes',true),
('saneamento','ligacao','excluir','saneamento.ligacao.excluir','Excluir ligaÃ§Ãµes',true),
...
('saneamento','hidrometro','visualizar','saneamento.hidrometro.visualizar','Visualizar hidrÃ´metros',true),
('saneamento','hidrometro','criar','saneamento.hidrometro.criar','Criar hidrÃ´metros',true),
('saneamento','hidrometro','editar','saneamento.hidrometro.editar','Editar hidrÃ´metros',true),
('saneamento','hidrometro','excluir','saneamento.hidrometro.excluir','Excluir hidrÃ´metros',true),
...
('saneamento','arrecadacao','visualizar','saneamento.arrecadacao.visualizar','Visualizar arrecadaÃ§Ã£o',true),
('saneamento','arrecadacao','criar','saneamento.arrecadacao.criar','Criar arrecadaÃ§Ã£o',true),
...
('saneamento','laboratorio','visualizar','saneamento.laboratorio.visualizar','Visualizar laboratÃ³rio',true),
...
-- MÃ³dulo AssistÃªncia Social Base - schema Ãºnico sigov, idempotente.
create extension if not exists pgcrypto;
...
insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo) values ('social','AssistÃªncia Social','MÃ³dulo base de AssistÃªncia Social','FinalÃ­stico',60,'/Social/Dashboard','bi-people',true) on conflict (codigo) do nothing;
insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo) values ('assistencia_social','AssistÃªncia Social','Alias de licenciamento para AssistÃªncia Social','FinalÃ­stico',61,'/Social/Dashboard','bi-people',true) on conflict (codigo) do nothing;
insert into sigov.permissao (modulo, recurso, acao, chave, descricao, ativo) values
('social','dashboard','visualizar','social.dashboard.visualizar','Visualizar dashboard social',true),('social','unidade','visualizar','social.unidade.visualizar','Visualizar unidades sociais',true),('social','unidade','criar','social.unidade.criar','Criar unidade social',true),('social','unidade','editar','social.unidade.editar','Editar unidade social',true),('social','unidade','excluir','social.unidade.excluir','Excluir unidade social',true),('social','familia','visualizar','social.familia.visualizar','Visualizar famÃ­lias',true),('social','familia','criar','social.familia.criar','Criar famÃ­lia',true),('social','familia','editar','social.familia.editar','Editar famÃ­lia',true),('social','familia','excluir','social.familia.excluir','Excluir famÃ­lia',true),('social','familia','visualizar_dados_completos','social.familia.visualizar_dados_completos','Visualizar dados completos de famÃ­lia',true),('social','pessoa','visualizar','social.pessoa.visualizar','Visualizar pessoas sociais',true),('social','pessoa','criar','social.pessoa.criar','Criar pessoa social',true),('social','pessoa','editar','social.pessoa.editar','Editar pessoa social',true),('social','pessoa','excluir','social.pessoa.excluir','Excluir pessoa social',true),('social','cadastro','criar','social.cadastro.criar','Criar cadastro social',true),('social','vulnerabilidade','criar','social.vulnerabilidade.criar','Registrar vulnerabilidade',true),('social','programa','visualizar','social.programa.visualizar','Visualizar programas',true),('social','programa','criar','social.programa.criar','Criar programa',true),('social','programa','editar','social.programa.editar','Editar programa',true),('social','programa','excluir','social.programa.excluir','Excluir programa',true),('social','beneficio','visualizar','social.beneficio.visualizar','Visualizar benefÃ­cios',true),('social','beneficio','criar','social.beneficio.criar','Criar benefÃ­cio',true),('social','beneficio','editar','social.beneficio.editar','Editar benefÃ­cio',true),('social','beneficio','conceder','social.beneficio.conceder','Conceder benefÃ­cio',true),('social','beneficio','autorizar','social.beneficio.autorizar','Autorizar benefÃ­cio',true),('social','beneficio','entregar','social.beneficio.entregar','Entregar benefÃ­cio',true),('social','atendimento','visualizar','social.atendimento.visualizar','Visualizar atendimentos',true),('social','atendimento','criar','social.atendimento.criar','Criar atendimento',true),('social','atendimento','encaminhar','social.atendimento.encaminhar','Criar encaminhamento',true),('social','visita','visualizar','social.visita.visualizar','Visualizar visitas',true),('social','visita','criar','social.visita.criar','Criar visita',true),('social','parecer','visualizar','social.parecer.visualizar','Visualizar pareceres',true),('social','parecer','criar','social.parecer.criar','Criar parecer',true),('social','parecer','visualizar_sigiloso','social.parecer.visualizar_sigiloso','Visualizar parecer sigiloso',true),('social','acompanhamento','visualizar','social.acompanhamento.visualizar','Visualizar acompanhamento',true),('social','acompanhamento','criar','social.acompanhamento.criar','Criar acompanhamento',true),('social','acompanhamento','encerrar','social.acompanhamento.encerrar','Encerrar acompanhamento',true),('social','vigilancia','visualizar','social.vigilancia.visualizar','Visualizar vigilÃ¢ncia',true),('social','vigilancia','criar','social.vigilancia.criar','Criar vigilÃ¢ncia',true),('social','exportar','executar','social.exportar','Exportar dados sociais',true) on conflict (modulo, recurso, acao) do nothing;
-- >>> database/postgres/migrations/025_integracoes_outbox_webhooks_base.sql
-- SIGOV - IntegraÃ§Ãµes oficiais, outbox avanÃ§ado, webhooks, API credentials e estruturas Gov.br/ICP-Brasil.
-- Migration incremental e idempotente; todas as tabelas permanecem no schema Ãºnico sigov.
...
insert into sigov.modulo_saas (codigo,nome,descricao,categoria,ordem,rota_base,icone,ativo)
values ('integracao','IntegraÃ§Ãµes','IntegraÃ§Ãµes oficiais, API credentials, webhooks, outbox, Gov.br e ICP-Brasil estrutural.','Plataforma',90,'/Integracoes','bi-diagram-3',true)
...
('integracao','integracao.sistema.visualizar','integracao.sistema','visualizar','Visualizar sistemas de integraÃ§Ã£o',true),
('integracao','integracao.sistema.criar','integracao.sistema','criar','Criar sistemas de integraÃ§Ã£o',true),
('integracao','integracao.sistema.editar','integracao.sistema','editar','Editar sistemas de integraÃ§Ã£o',true),
('integracao','integracao.sistema.excluir','integracao.sistema','excluir','Excluir sistemas de integraÃ§Ã£o',true),
...
('integracao','integracao.dashboard.visualizar','integracao.dashboard','visualizar','Visualizar dashboard de integraÃ§Ãµes',true),
('integracao','integracao.exportar','integracao.exportacao','exportar','Exportar dados de integraÃ§Ãµes',true)
...
-- Agro e Desenvolvimento Rural - fundaÃ§Ã£o, georreferenciamento estrutural e dashboard inicial.
-- Schema Ãºnico sigov; sem PostGIS obrigatÃ³rio; Dapper/PostgreSQL-ready.

insert into sigov.modulo_saas (codigo, nome, descricao, categoria, ordem, rota_base, icone, ativo)
values ('agro', 'Agro e Desenvolvimento Rural', 'FundaÃ§Ã£o do mÃ³dulo Agro com georreferenciamento estrutural e dashboard inicial.', 'PolÃ­ticas pÃºblicas', 160, '/Agro/Dashboard', 'bi-tree', true)
...
('agro','geo','visualizar','agro.geo.visualizar','Visualizar camadas e feiÃ§Ãµes geogrÃ¡ficas do Agro.',true),
('agro','geo','criar','agro.geo.criar','Criar camadas e feiÃ§Ãµes geogrÃ¡ficas do Agro.',true),
('agro','geo','editar','agro.geo.editar','Editar camadas e feiÃ§Ãµes geogrÃ¡ficas do Agro.',true),
('agro','geo','excluir','agro.geo.excluir','Excluir camadas e feiÃ§Ãµes geogrÃ¡ficas do Agro.',true),
('agro','geo','exportar','agro.geo.exportar','Exportar dados geogrÃ¡ficos do Agro em GeoJSON.',true)
on conflict (modulo, recurso, acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;

insert into sigov.feature_flag_def (codigo, nome, descricao, modulo, ativo)
values
('agro.dashboard','Dashboard Agro','Habilita o dashboard inicial do mÃ³dulo Agro.','agro',true),
('agro.geo','Georreferenciamento Agro','Habilita camadas e feiÃ§Ãµes geogrÃ¡ficas estruturais do Agro.','agro',true),
('agro.exportacao_geojson','ExportaÃ§Ã£o GeoJSON Agro','Habilita exportaÃ§Ã£o estrutural GeoJSON do Agro.','agro',true)
...
-- FundaÃ§Ã£o SaaS parametrizÃ¡vel do sigov: schema Ãºnico sigov, Dapper/PostgreSQL e isolamento por tenant.
...
('ADMINISTRADOR_GERAL', 'Administrador Geral', 'Administra todo o sigov e todos os tenants com auditoria obrigatÃ³ria.', 1000, true, false),
('ADMINISTRADOR_TENANT', 'Administrador do Tenant', 'Administra apenas o prÃ³prio tenant.', 900, false, true),
('ADMINISTRADOR_ENTIDADE', 'Administrador de Entidade', 'Administra uma entidade especÃ­fica dentro do tenant.', 800, false, false),
('COORDENADOR', 'Coordenador', 'Coordena mÃ³dulos, setores, equipes ou unidades conforme permissÃµes.', 700, false, false),
('DIRETOR', 'Diretor', 'Gerencia unidade, escola, secretaria, departamento ou Ã¡rea conforme escopo.', 600, false, false),
('SERVIDOR', 'Servidor', 'Executa tarefas operacionais prÃ³prias ou atribuÃ­das.', 500, false, false),
('OPERADOR', 'Operador', 'Executa permissÃµes operacionais explÃ­citas.', 400, false, false),
('CONSULTA', 'Consulta', 'UsuÃ¡rio somente leitura conforme escopo.', 300, false, false),
('AUDITOR', 'Auditor', 'Acessa auditoria e conformidade conforme permissÃ£o.', 200, false, false),
('SUPORTE', 'Suporte', 'Suporte tÃ©cnico controlado e auditado.', 100, false, false)
...
('ESSENCIAL','Essencial','FundaÃ§Ã£o mÃ­nima da plataforma.','["core","seguranca","auditoria","lgpd","suporte"]'::jsonb),
('FINANCEIRO_TRIBUTARIO','Financeiro e TributÃ¡rio','GestÃ£o fiscal, arrecadaÃ§Ã£o e relatÃ³rios.','["financeiro","tributario","relatorios"]'::jsonb),
('GESTAO_ADMINISTRATIVA','GestÃ£o Administrativa','Backoffice administrativo municipal.','["processos","compras","contratos","almoxarifado","patrimonio","frotas","obras"]'::jsonb),
('SOCIAL_SAUDE_EDUCACAO','Social, SaÃºde e EducaÃ§Ã£o','PolÃ­ticas pÃºblicas integradas.','["educacao","saude","social"]'::jsonb),
('AGRO_RURAL','Agro Rural','Base futura rural integrada.','["agro","frotas","obras","tributario","relatorios"]'::jsonb),
('COMPLETO','Completo','Todos os mÃ³dulos integrados do sigov.','["core","seguranca","auditoria","lgpd","processos","financeiro","tributario","compras","contratos","almoxarifado","patrimonio","frotas","obras","rh","educacao","saude","saneamento","social","relatorios","transparencia","integracoes","suporte","operacao","agro"]'::jsonb)
...
('agro','talhao','visualizar','agro.talhao.visualizar','Visualizar talhÃµes.',true),('agro','talhao','criar','agro.talhao.criar','Criar talhÃµes.',true),('agro','talhao','editar','agro.talhao.editar','Editar talhÃµes.',true),
...
('agro','producao','visualizar','agro.producao.visualizar','Visualizar produÃ§Ã£o agrÃ­cola.',true),('agro','producao','criar','agro.producao.criar','Criar produÃ§Ã£o agrÃ­cola.',true),('agro','producao','editar','agro.producao.editar','Editar produÃ§Ã£o agrÃ­cola.',true),('agro','producao','excluir','agro.producao.excluir','Excluir produÃ§Ã£o agrÃ­cola.',true)
on conflict (chave) do update set descricao=excluded.descricao, ativo=true, is_deleted=false;

insert into sigov.feature_flag_def (codigo, nome, descricao, modulo, ativo)
values ('agro.produtores','Produtores rurais','Habilita cadastro de produtores rurais.','agro',true),('agro.producao','ProduÃ§Ã£o agrÃ­cola','Habilita cadastros produtivos agrÃ­colas.','agro',true)
...
select t.id, e.id, x.codigo, x.nome, x.tipo, x.unidade, null from sigov.tenant t join sigov.entidade e on e.tenant_id=t.id cross join (values ('MILHO','Milho','GRAOS','kg'),('FEIJAO','FeijÃ£o','GRAOS','kg'),('MANDIOCA','Mandioca','RAIZES_TUBERCULOS','kg'),('ARROZ','Arroz','GRAOS','kg'),('HORTALICAS','HortaliÃ§as','HORTALICAS','kg'),('FRUTAS','Frutas','FRUTAS','kg'),('PASTAGEM','Pastagem','PASTAGEM','ha')) as x(codigo,nome,tipo,unidade) where t.ambiente='Development' and not exists (select 1 from sigov.agro_cultura c where c.tenant_id=t.id and c.entidade_id=e.id and c.codigo=x.codigo and c.is_deleted=false);
...
-- Agro Parte 4: programas, benefÃ­cios, insumos e patrulha mecanizada.
-- Schema Ãºnico sigov; migration idempotente; sem schemas agro/rural/geo.

insert into sigov.feature_flag_def (codigo, nome, descricao, modulo, ativo)
values
('agro.programas_rurais','Programas rurais','Habilita programas rurais do Agro.','agro',true),
('agro.beneficios_rurais','BenefÃ­cios rurais','Habilita benefÃ­cios e concessÃµes rurais.','agro',true),
('agro.insumos','Insumos Agro','Habilita insumos e distribuiÃ§Ãµes rurais.','agro',true),
('agro.patrulha_mecanizada','Patrulha mecanizada','Habilita patrulha mecanizada rural.','agro',true),
('agro.maquinas','MÃ¡quinas rurais','Habilita mÃ¡quinas e implementos rurais.','agro',true),
('agro.servicos_maquina','ServiÃ§os de mÃ¡quinas','Habilita serviÃ§os de mÃ¡quinas rurais.','agro',true)
on conflict (codigo) do update set nome=excluded.nome, descricao=excluded.descricao, modulo=excluded.modulo, ativo=true;
...
('agro','beneficio','visualizar','agro.beneficio.visualizar','Visualizar benefÃ­cios rurais.',true),('agro','beneficio','criar','agro.beneficio.criar','Criar benefÃ­cios rurais.',true),('agro','beneficio','editar','agro.beneficio.editar','Editar benefÃ­cios rurais.',true),('agro','beneficio','excluir','agro.beneficio.excluir','Excluir benefÃ­cios rurais.',true),('agro','beneficio','conceder','agro.beneficio.conceder','Solicitar concessÃµes de benefÃ­cio rural.',true),('agro','beneficio','autorizar','agro.beneficio.autorizar','Autorizar concessÃµes de benefÃ­cio rural.',true),('agro','beneficio','entregar','agro.beneficio.entregar','Entregar concessÃµes de benefÃ­cio rural.',true),('agro','beneficio','indeferir','agro.beneficio.indeferir','Indeferir concessÃµes de benefÃ­cio rural.',true),('agro','beneficio','cancelar','agro.beneficio.cancelar','Cancelar concessÃµes de benefÃ­cio rural.',true),
...
('agro','maquina','visualizar','agro.maquina.visualizar','Visualizar mÃ¡quinas rurais.',true),('agro','maquina','criar','agro.maquina.criar','Criar mÃ¡quinas rurais.',true),('agro','maquina','editar','agro.maquina.editar','Editar mÃ¡quinas rurais.',true),('agro','maquina','excluir','agro.maquina.excluir','Excluir mÃ¡quinas rurais.',true),
...
('agro','servico_maquina','visualizar','agro.servico_maquina.visualizar','Visualizar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','criar','agro.servico_maquina.criar','Criar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','editar','agro.servico_maquina.editar','Editar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','agendar','agro.servico_maquina.agendar','Agendar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','executar','agro.servico_maquina.executar','Executar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','cancelar','agro.servico_maquina.cancelar','Cancelar serviÃ§os de mÃ¡quinas.',true),('agro','servico_maquina','excluir','agro.servico_maquina.excluir','Excluir serviÃ§os de mÃ¡quinas.',true)
on conflict (modulo, recurso, acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;
...
select t.id, e.id, x.codigo, x.nome, x.tipo, 'Seed Development Agro Parte 4', null from sigov.tenant t join sigov.entidade e on e.tenant_id=t.id cross join (values ('CORTE_TERRA','Corte de Terra','CORTE_TERRA'),('DIST_SEMENTES','DistribuiÃ§Ã£o de Sementes','DISTRIBUICAO_SEMENTES'),('DIST_MUDAS','DistribuiÃ§Ã£o de Mudas','DISTRIBUICAO_MUDAS'),('PATRULHA_MEC','Patrulha Mecanizada','PATRULHA_MECANIZADA')) as x(codigo,nome,tipo)
...
select t.id, e.id, x.codigo, x.nome, x.tipo, x.unidade, false, null from sigov.tenant t join sigov.entidade e on e.tenant_id=t.id cross join (values ('SEMENTE','Semente','SEMENTE','kg'),('MUDA','Muda','MUDA','un'),('CALCARIO','CalcÃ¡rio','CALCARIO','kg'),('ADUBO','Adubo','ADUBO','kg')) as x(codigo,nome,tipo,unidade)
...
create table if not exists sigov.b2b_tenant_white_label (
    ...
    nome_plataforma varchar(160) not null default 'PlantÃ£oPro',
    nome_comercial varchar(160) not null default 'PlantÃ£oPro',
...
values
('ESSENCIAL', 'Plano Essencial', 'OperaÃ§Ã£o inicial para escalas e plantÃµes mÃ©dicos.', 'Hospitais e clÃ­nicas em inÃ­cio de operaÃ§Ã£o digital', 499, false, false, 5, 20, 2, 100, 'Suporte padrÃ£o', 10),
('PROFISSIONAL', 'Plano Profissional', 'OperaÃ§Ã£o assistida com API limitada, relatÃ³rios avanÃ§ados e white label bÃ¡sico.', 'OperaÃ§Ãµes multiunidade em crescimento', 1499, true, true, 20, 100, 10, 500, 'Suporte prioritÃ¡rio', 20),
('ENTERPRISE_WHITE_LABEL', 'Enterprise White Label', 'White label completo, domÃ­nio customizado, API, webhooks, BI e SLA.', 'Redes de saÃºde e operaÃ§Ãµes B2B', 4990, true, true, 999999, 999999, 999999, 999999, 'SLA contratual', 30),
('REVENDEDOR', 'Plano Revendedor', 'Console parceiro, tenants vinculados, margem, comissÃ£o e repasses.', 'Parceiros e consultorias', 7990, true, true, 999999, 999999, 999999, 999999, 'SLA B2B para parceiros', 40),
('CUSTOM', 'Plano Custom', 'Contrato personalizado, integraÃ§Ãµes especÃ­ficas e infraestrutura dedicada opcional.', 'Projetos sob proposta', 0, true, true, 999999, 999999, 999999, 999999, 'SLA customizado', 50)
...
values
('One page comercial PlantÃ£oPro', 'ONE_PAGE', 'parceiro', 'Resumo comercial para hospitais, redes de saÃºde e parceiros.'),
('Roteiro de demonstraÃ§Ã£o white label', 'DEMO', 'interno', 'Fluxo de planos, self-service, white label, API, suporte e monitoramento.'),
('ArgumentÃ¡rio para CTO', 'ARGUMENTARIO', 'parceiro', 'SeguranÃ§a, isolamento por tenant, API keys, LGPD, auditoria e observabilidade.'),
('Proposta comercial modelo', 'PROPOSTA', 'interno', 'Modelo de proposta B2B com setup, SLA, revenda e customizaÃ§Ãµes.')
on conflict do nothing;
...
values
('ESSENCIAL','Essencial','Core, SeguranÃ§a, Auditoria/LGPD e suporte bÃ¡sico.',true,false,10,'PUBLICO',null,'MENSAL',20,2,10240,false,false),
('FINANCEIRO_TRIBUTARIO','Financeiro e TributÃ¡rio','Pacote para financeiro, tributÃ¡rio e relatÃ³rios.',true,false,20,'PUBLICO',null,'MENSAL',50,5,20480,false,false),
('GESTAO_ADMINISTRATIVA','GestÃ£o Administrativa','Processos, compras, contratos, almoxarifado, patrimÃ´nio, frotas e obras.',true,false,30,'PUBLICO',null,'MENSAL',100,10,51200,false,false),
('SOCIAL_SAUDE_EDUCACAO','Social, SaÃºde e EducaÃ§Ã£o','EducaÃ§Ã£o, saÃºde e assistÃªncia social integradas.',true,false,40,'PUBLICO',null,'MENSAL',150,20,102400,false,false),
('AGRO_RURAL','Agro Rural','Agro, frotas, obras e relatÃ³rios com tributÃ¡rio opcional.',true,false,50,'PUBLICO',null,'MENSAL',60,10,51200,false,false),
('COMPLETO','Completo','Todos os mÃ³dulos do sigov com white label e domÃ­nio customizado.',true,true,60,'PUBLICO',null,'MENSAL',300,50,204800,true,true),
('ENTERPRISE','Enterprise','Plano personalizado para operaÃ§Ã£o municipal avanÃ§ada.',true,false,70,'ENTERPRISE',null,'PERSONALIZADA',null,null,null,true,true)
...
values
('USUARIOS_EXTRAS','UsuÃ¡rios extras','Pacote adicional de usuÃ¡rios.','USUARIOS_EXTRAS',null,null,'MENSAL'),
('ARMAZENAMENTO_EXTRA','Armazenamento extra','Armazenamento adicional.','ARMAZENAMENTO_EXTRA',null,null,'MENSAL'),
('WHITE_LABEL','White label','Identidade visual prÃ³pria do tenant.','WHITE_LABEL',null,null,'MENSAL'),
('DOMINIO_CUSTOMIZADO','DomÃ­nio customizado','DomÃ­nio estrutural customizado.','DOMINIO_CUSTOMIZADO',null,null,'MENSAL'),
('SUPORTE_PREMIUM','Suporte premium','Suporte tÃ©cnico ampliado.','SUPORTE_PREMIUM',null,null,'MENSAL'),
('TRIBUTARIO_EXTRA','TributÃ¡rio opcional','MÃ³dulo TributÃ¡rio adicional.','MODULO_EXTRA','tributario',null,'MENSAL')
...
from sigov.saas_plano p
join (values
('usuarios','UsuÃ¡rios',null,'usuarios',false),('entidades','Entidades',null,'entidades',false),('armazenamento_mb','Armazenamento',null,'MB',false),('api_requests_mes','RequisiÃ§Ãµes API/mÃªs',50000,'requests',false),('relatorios_agendados','RelatÃ³rios agendados',10,'relatÃ³rios',false),('integracoes','IntegraÃ§Ãµes',3,'integraÃ§Ãµes',false),('white_label','White label',null,null,false),('dominio_customizado','DomÃ­nio customizado',null,null,false)
...
('COORDENADOR','Coordenador','COORDENADOR','Coordena mÃ³dulos, setores ou unidades permitidas.', '[]'::jsonb),
...
('AUDITOR','Auditor','AUDITOR','Acessa auditoria e conformidade conforme permissÃ£o.', '["saas.assinaturas.visualizar"]'::jsonb)
...
-- Agro Parte 5 - Estradas vicinais, pontos crÃ­ticos, feiras, agroindÃºstrias e compras AF.
-- Idempotente; schema Ãºnico sigov; sem PostGIS obrigatÃ³rio; sem integraÃ§Ãµes financeiras/tributÃ¡rias reais.

insert into sigov.feature_flag_def (codigo, nome, descricao, modulo, ativo) values
('agro.estradas_vicinais','Estradas Vicinais','Habilita cadastro e gestÃ£o de estradas vicinais.','agro',true),
('agro.pontos_criticos','Pontos CrÃ­ticos Rurais','Habilita pontes, bueiros, passagens molhadas e pontos crÃ­ticos.','agro',true),
('agro.ocorrencias_rurais','OcorrÃªncias Rurais','Habilita ocorrÃªncias rurais estruturais.','agro',true),
('agro.feiras','Feiras e Mercados Rurais','Habilita feiras, feirantes, bancas e autorizaÃ§Ãµes.','agro',true),
('agro.agroindustrias','AgroindÃºstrias','Habilita agroindÃºstrias municipais estruturais.','agro',true),
('agro.inspecao_municipal','InspeÃ§Ã£o Municipal','Habilita inspeÃ§Ã£o municipal estrutural.','agro',true),
...
('agro','ponto_critico','visualizar','agro.ponto_critico.visualizar','Visualizar pontos crÃ­ticos.',true),('agro','ponto_critico','criar','agro.ponto_critico.criar','Criar pontos crÃ­ticos.',true),('agro','ponto_critico','editar','agro.ponto_critico.editar','Editar pontos crÃ­ticos.',true),('agro','ponto_critico','resolver','agro.ponto_critico.resolver','Resolver pontos crÃ­ticos.',true),
('agro','ocorrencia','visualizar','agro.ocorrencia.visualizar','Visualizar ocorrÃªncias rurais.',true),('agro','ocorrencia','criar','agro.ocorrencia.criar','Criar ocorrÃªncias rurais.',true),('agro','ocorrencia','editar','agro.ocorrencia.editar','Editar ocorrÃªncias rurais.',true),('agro','ocorrencia','resolver','agro.ocorrencia.resolver','Resolver ocorrÃªncias rurais.',true),('agro','ocorrencia','cancelar','agro.ocorrencia.cancelar','Cancelar ocorrÃªncias rurais.',true),
('agro','manutencao','visualizar','agro.manutencao.visualizar','Visualizar manutenÃ§Ãµes rurais.',true),('agro','manutencao','criar','agro.manutencao.criar','Criar manutenÃ§Ãµes rurais.',true),('agro','manutencao','executar','agro.manutencao.executar','Executar manutenÃ§Ãµes rurais.',true),('agro','manutencao','cancelar','agro.manutencao.cancelar','Cancelar manutenÃ§Ãµes rurais.',true),
...
('agro','agroindustria','visualizar','agro.agroindustria.visualizar','Visualizar agroindÃºstrias.',true),('agro','agroindustria','criar','agro.agroindustria.criar','Criar agroindÃºstrias.',true),('agro','agroindustria','editar','agro.agroindustria.editar','Editar agroindÃºstrias.',true),('agro','agroindustria','excluir','agro.agroindustria.excluir','Excluir agroindÃºstrias.',true),
('agro','inspecao','visualizar','agro.inspecao.visualizar','Visualizar inspeÃ§Ãµes municipais.',true),('agro','inspecao','criar','agro.inspecao.criar','Criar inspeÃ§Ãµes municipais.',true),('agro','inspecao','concluir','agro.inspecao.concluir','Concluir inspeÃ§Ãµes municipais.',true),
...
select t.id, e.id, c.codigo, c.nome, c.tipo, c.nome, false from sigov.tenant t join sigov.entidade e on e.tenant_id=t.id cross join (values ('ESTRADAS_VICINAIS','Estradas Vicinais','ESTRADAS'),('PONTOS_CRITICOS','Pontos CrÃ­ticos','PONTOS_CRITICOS'),('OCORRENCIAS_RURAIS','OcorrÃªncias Rurais','OCORRENCIAS'),('FEIRAS_RURAIS','Feiras','FEIRAS'),('AGROINDUSTRIAS','AgroindÃºstrias','AGROINDUSTRIAS')) c(codigo,nome,tipo) where t.ativo=true and t.is_deleted=false on conflict do nothing;
...
-- Eventos estruturais previstos para sigov.agro_evento/sigov.fila_evento, sem integraÃ§Ã£o real nesta etapa:
...
-- AgroGeoFeicaoAtualizada. Payload mÃ­nimo: tenantId, entidadeId, exercicioId, origem, origemId, tipoEvento, correlationId.
...
from sigov.tenant t
cross join (values
('total_produtores','Total de produtores','PRODUTORES',null),('total_propriedades','Total de propriedades','PROPRIEDADES',null),('area_produtiva_ha','Ãrea produtiva','PROPRIEDADES','ha'),('total_culturas','Total de culturas','PRODUCAO',null),('producao_estimada','ProduÃ§Ã£o estimada','PRODUCAO',null),('producao_realizada','ProduÃ§Ã£o realizada','PRODUCAO',null),('total_rebanhos','Total de rebanhos','PECUARIA',null),('total_animais','Total de animais','PECUARIA',null),('visitas_tecnicas_mes','Visitas tÃ©cnicas no mÃªs','ASSISTENCIA_TECNICA',null),('beneficios_entregues_mes','BenefÃ­cios entregues no mÃªs','PROGRAMAS',null),('servicos_maquina_executados_mes','ServiÃ§os de mÃ¡quina executados no mÃªs','PATRULHA_MECANIZADA',null),('estradas_vicinais_km','Estradas vicinais em km','ESTRADAS','km'),('pontos_criticos_abertos','Pontos crÃ­ticos abertos','ESTRADAS',null),('feiras_ativas','Feiras ativas','FEIRAS',null),('agroindustrias_ativas','AgroindÃºstrias ativas','AGROINDUSTRIA',null),('compras_af_mes','Compras da agricultura familiar no mÃªs','COMPRAS_AF',null)
...
('agro_produtor','pessoa_id','Pessoa do produtor','ReferÃªncia interna ao cadastro de pessoa; nunca expor em dados abertos.','PRODUTORES',true,false,false,'ID_INTERNO'),
('agro_propriedade','endereco_json','EndereÃ§o da propriedade','EndereÃ§o completo deve ser mascarado em exportaÃ§Ãµes pÃºblicas.','PROPRIEDADES',true,false,false,'ENDERECO_MASCARADO'),
('agro_agroindustria','endereco_json','EndereÃ§o da agroindÃºstria','EndereÃ§o completo deve ser mascarado em exportaÃ§Ãµes pÃºblicas.','AGROINDUSTRIA',true,false,false,'ENDERECO_MASCARADO'),
('agro_compra_agricultura_familiar','produto','Produto adquirido','Campo publicÃ¡vel apenas agregado por produto.','COMPRAS_AF',false,false,true,null)
on conflict do nothing;
...
values ('Administrador Geral', 'Perfil global para administraÃ§Ã£o SaaS inicial do SIGOV.', 'ADMINISTRADOR_GERAL', true)
on conflict do nothing;
...
values
    ...
    ('saas', 'modulos', 'visualizar', 'saas.modulos.visualizar', 'Visualizar mÃ³dulos SaaS', true),
    ('saas', 'modulos', 'gerenciar', 'saas.modulos.gerenciar', 'Gerenciar mÃ³dulos contratados', true),
    ('operacao', 'health', 'visualizar', 'operacao.health.visualizar', 'Visualizar saÃºde do ambiente', true)
on conflict do nothing;

insert into sigov.usuario (tenant_id, login, email, senha_hash, nome, ativo, bloqueado, deve_alterar_senha, observacao)
select t.id, 'admin', 'admin@sigov.local', 'SIGOV_PBKDF2_V1$100000$UG9zdEJ1aWxkMDFTYWx0IQ==$C87vVWxqrRbbxe7IoBsv0wuI7NDFYnsFlrAxvqlherg=', 'Administrador Geral', true, false, true, 'UsuÃ¡rio administrador inicial de desenvolvimento com senha documentada em docs/ambiente-local.md'
from sigov.tenant t
...
-- EvoluÃ§Ã£o PÃ³s-Build 03 - Planos SaaS, implantaÃ§Ã£o guiada e base tributÃ¡ria parametrizÃ¡vel.
-- Migration idempotente: somente CREATE/ALTER IF NOT EXISTS, Ã­ndices IF NOT EXISTS e seeds ON CONFLICT.
...
values
('STARTER','Starter','Plano inicial para pequenos Ã³rgÃ£os e implantaÃ§Ã£o piloto.','MENSAL',0,'BRL','MENSAL',10,1,1024,false,false,true,false,true,10),
('GOV_BASIC','Governo BÃ¡sico','Plano para secretarias e Ã³rgÃ£os com gestÃ£o administrativa.','MENSAL',0,'BRL','MENSAL',50,1,10240,true,false,true,true,true,20),
('GOV_PLUS','Governo Plus','Plano completo para prefeituras e estruturas multiÃ¡reas.','MENSAL',0,'BRL','MENSAL',200,3,51200,true,false,true,true,true,30),
('ENTERPRISE','Enterprise','Plano corporativo com mÃ³dulos ilimitados e domÃ­nio personalizado.','MENSAL',0,'BRL','MENSAL',null,null,null,true,true,false,false,true,40)
...
insert into sigov.saas_plano_limite (plano_id,codigo,nome,valor,unidade,ilimitado)
select p.id, 'usuarios', 'UsuÃ¡rios ativos', p.limite_usuarios, 'usuarios', p.limite_usuarios is null from sigov.saas_plano p
...
('saas','planos','modulos','saas.planos.modulos','Configurar mÃ³dulos de planos SaaS',true),
('saas','planos','limites','saas.planos.limites','Configurar limites de planos SaaS',true),
('tributario','dashboard','visualizar','tributario.dashboard.visualizar','Visualizar dashboard tributÃ¡rio',true),
('tributario','configuracao','visualizar','tributario.configuracao.visualizar','Visualizar configuraÃ§Ã£o tributÃ¡ria',true),
('tributario','configuracao','editar','tributario.configuracao.editar','Editar configuraÃ§Ã£o tributÃ¡ria',true),
('tributario','tipos','visualizar','tributario.tipos.visualizar','Visualizar tipos tributÃ¡rios',true),
('tributario','tipos','editar','tributario.tipos.editar','Editar tipos tributÃ¡rios',true),
('tributario','campos','visualizar','tributario.campos.visualizar','Visualizar campos dinÃ¢micos tributÃ¡rios',true),
('tributario','campos','editar','tributario.campos.editar','Editar campos dinÃ¢micos tributÃ¡rios',true),
...
('tributario','imoveis','visualizar','tributario.imoveis.visualizar','Visualizar imÃ³veis',true),
('tributario','imoveis','criar','tributario.imoveis.criar','Criar imÃ³veis',true),
('tributario','imoveis','editar','tributario.imoveis.editar','Editar imÃ³veis',true),
('tributario','economicos','visualizar','tributario.economicos.visualizar','Visualizar econÃ´micos',true),
('tributario','economicos','criar','tributario.economicos.criar','Criar econÃ´micos',true),
('tributario','economicos','editar','tributario.economicos.editar','Editar econÃ´micos',true)
on conflict (modulo,recurso,acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;
...
from sigov.tenant t
cross join (values
('orgao.nome',null,'string','Nome do Ã³rgÃ£o',false),
('orgao.documento',null,'string','Documento do Ã³rgÃ£o',false),
('orgao.email',null,'string','E-mail institucional',false),
('orgao.telefone',null,'string','Telefone institucional',false),
('orgao.endereco',null,'string','EndereÃ§o institucional',false),
('sistema.timezone','America/Sao_Paulo','string','Timezone padrÃ£o',false),
('sistema.locale','pt-BR','string','Locale padrÃ£o',false),
('sistema.moeda','BRL','string','Moeda padrÃ£o',false),
('lgpd.mascara_dados','true','boolean','Aplicar mÃ¡scara LGPD',false),
('auditoria.retencao_dias','365','number','RetenÃ§Ã£o da auditoria',false)
...
from sigov.tenant t
cross join (values
('CONTRIBUINTE','Contribuinte','Cadastro parametrizÃ¡vel de contribuintes'),
('IMOVEL','ImÃ³vel','Cadastro imobiliÃ¡rio parametrizÃ¡vel'),
('ECONOMICO','EconÃ´mico','Cadastro econÃ´mico/mobiliÃ¡rio parametrizÃ¡vel')
) as v(codigo,nome,descricao)
on conflict (tenant_id,codigo) do nothing;
-- >>> database/postgres/migrations/20260610100000_pos_build_04_enterprise_modules.sql
-- SIGOV PÃ³s-Build 04: base comercial, ordem de serviÃ§o, manutenÃ§Ã£o industrial, estoque/compras e comÃ©rcio.
create schema if not exists sigov;
...
insert into sigov.permissao (modulo,recurso,acao,chave,descricao,ativo)
values
('comercial','*','gerenciar','comercial.*','Gerenciar mÃ³dulo Comercial/CRM',true),
('os','*','gerenciar','os.*','Gerenciar mÃ³dulo de Ordem de ServiÃ§o',true),
('industrial','*','gerenciar','industrial.*','Gerenciar mÃ³dulo de ManutenÃ§Ã£o Industrial',true),
('estoque','*','gerenciar','estoque.*','Gerenciar mÃ³dulo de Estoque',true),
('compras','*','gerenciar','compras.*','Gerenciar mÃ³dulo de Compras',true),
('comercio','*','gerenciar','comercio.*','Gerenciar mÃ³dulo de ComÃ©rcio',true)
on conflict (modulo,recurso,acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;
-- >>> database/postgres/migrations/20260610120000_pos_build_05_comercio_varejo_atacado_pdv_caixa_financeiro.sql
-- SIGOV PÃ³s-Build 05: ComÃ©rcio varejista/atacadista avanÃ§ado, PDV, caixa, estoque integrado e financeiro inicial.
-- Migration idempotente: CREATE TABLE/INDEX IF NOT EXISTS e seeds com ON CONFLICT, sem remoÃ§Ã£o destrutiva.
create schema if not exists sigov;
...
insert into sigov.permissao (modulo,recurso,acao,chave,descricao,ativo) values
('comercio','dashboard','visualizar','comercio.dashboard.visualizar','Visualizar dashboard comercial avanÃ§ado',true),
...
('comercio','orcamentos','visualizar','comercio.orcamentos.visualizar','Visualizar orÃ§amentos',true),
('comercio','orcamentos','criar','comercio.orcamentos.criar','Criar orÃ§amentos',true),
('comercio','orcamentos','aprovar','comercio.orcamentos.aprovar','Aprovar orÃ§amentos',true),
...
('comercio','tabelas','visualizar','comercio.tabelas.visualizar','Visualizar tabelas de preÃ§o',true),
('comercio','tabelas','editar','comercio.tabelas.editar','Editar tabelas de preÃ§o',true),
('comercio','comissoes','visualizar','comercio.comissoes.visualizar','Visualizar comissÃµes',true),
('comercio','comissoes','calcular','comercio.comissoes.calcular','Calcular comissÃµes',true),
...
insert into sigov.tenant_modulo_pacote (codigo, nome, descricao, modulos_json) values
('COMERCIO_STARTER','ComÃ©rcio Starter','Varejo com PDV, caixa e estoque.','["comercial","comercio_varejo","pdv","caixa","estoque_compras"]'::jsonb),
('COMERCIO_PLUS','ComÃ©rcio Plus','Varejo e atacado integrados ao estoque e financeiro inicial.','["comercial","comercio_varejo","comercio_atacado","pdv","caixa","estoque_compras","financeiro_empresarial"]'::jsonb),
('ATACADO_PRO','Atacado Pro','Pedidos, separaÃ§Ã£o e financeiro inicial para atacado.','["comercial","comercio_atacado","pedidos","estoque_compras","financeiro_empresarial"]'::jsonb),
('BUSINESS_FULL','Business Full','ComÃ©rcio, OS, manutenÃ§Ã£o industrial, estoque e financeiro inicial integrados.','["comercial","comercio_varejo","comercio_atacado","pdv","caixa","estoque_compras","ordem_servico","manutencao_industrial","financeiro_empresarial"]'::jsonb)
on conflict (codigo) do update set nome=excluded.nome, descricao=excluded.descricao, modulos_json=excluded.modulos_json, ativo=true;
...
('COMERCIO_STARTER','ComÃ©rcio Starter','Plano inicial de varejo com PDV, caixa e estoque.','MENSAL',0,'BRL','MENSAL',20,true,true,true,50),
('COMERCIO_PLUS','ComÃ©rcio Plus','Plano integrado para varejo e atacado com financeiro comercial inicial.','MENSAL',0,'BRL','MENSAL',60,true,true,true,60),
('ATACADO_PRO','Atacado Pro','Plano atacadista com pedidos, separaÃ§Ã£o, estoque e contas a receber.','MENSAL',0,'BRL','MENSAL',80,true,false,true,70),
('BUSINESS_FULL','Business Full','Plano empresarial completo com comÃ©rcio, OS, manutenÃ§Ã£o, estoque e financeiro inicial.','MENSAL',0,'BRL','MENSAL',200,false,false,true,80)
...
-- SIGOV PÃ³s-Build 06: IndÃºstria e ProduÃ§Ã£o avanÃ§ada, chÃ£o de fÃ¡brica e integraÃ§Ãµes.
-- Migration idempotente: CREATE TABLE/INDEX IF NOT EXISTS e seeds com ON CONFLICT, sem remoÃ§Ã£o destrutiva.
create schema if not exists sigov;
create extension if not exists pgcrypto;
...
('industria_producao','IndÃºstria e ProduÃ§Ã£o','ProduÃ§Ã£o por ordem, BOM, roteiro, chÃ£o de fÃ¡brica, qualidade e custos.','EMPRESARIAL',true),
('manutencao_industrial','ManutenÃ§Ã£o Industrial','Ativos, planos e manutenÃ§Ã£o industrial integrada.','EMPRESARIAL',true),
('ordem_servico','Ordem de ServiÃ§o','Ordens de serviÃ§o tÃ©cnicas e operacionais.','EMPRESARIAL',true),
...
('comercio_varejo','ComÃ©rcio Varejo','Varejo, balcÃ£o e operaÃ§Ãµes de loja.','EMPRESARIAL',true),
('comercio_atacado','ComÃ©rcio Atacado','Pedidos, tabelas e separaÃ§Ã£o atacadista.','EMPRESARIAL',true),
...
insert into sigov.tenant_modulo_pacote (codigo, nome, descricao, modulos_json) values
('INDUSTRIAL_STARTER','Industrial Starter','ProduÃ§Ã£o, estoque e ordem de serviÃ§o para iniciar operaÃ§Ã£o industrial.','["industria_producao","estoque_compras","ordem_servico"]'::jsonb),
('INDUSTRIAL_PLUS','Industrial Plus','ProduÃ§Ã£o integrada a manutenÃ§Ã£o, OS, compras, estoque e financeiro.','["industria_producao","manutencao_industrial","ordem_servico","estoque_compras","compras","financeiro_empresarial"]'::jsonb),
('FACTORY_FULL','Factory Full','Pacote fabril completo com comercial atacadista, produÃ§Ã£o, estoque e financeiro.','["industria_producao","manutencao_industrial","ordem_servico","estoque_compras","comercial","comercio_atacado","financeiro_empresarial"]'::jsonb),
('BUSINESS_FULL','Business Full','Pacote empresarial completo com comÃ©rcio, produÃ§Ã£o, OS, manutenÃ§Ã£o, estoque e financeiro.','["comercial","comercio_varejo","comercio_atacado","pdv","caixa","estoque_compras","ordem_servico","manutencao_industrial","industria_producao","financeiro_empresarial"]'::jsonb)
on conflict (codigo) do update set nome=excluded.nome, descricao=excluded.descricao, modulos_json=excluded.modulos_json, ativo=true;

insert into sigov.saas_plano (codigo,nome,descricao,tipo_plano,preco_base,moeda,periodicidade,limite_usuarios,publico,destaque,ativo,ordem) values
('INDUSTRIAL_STARTER','Industrial Starter','IndÃºstria por ordem com estoque e OS.','MENSAL',0,'BRL','MENSAL',40,true,true,true,90),
('INDUSTRIAL_PLUS','Industrial Plus','IndÃºstria integrada com manutenÃ§Ã£o, compras e financeiro.','MENSAL',0,'BRL','MENSAL',120,true,true,true,100),
('FACTORY_FULL','Factory Full','OperaÃ§Ã£o fabril e comercial atacadista completa.','MENSAL',0,'BRL','MENSAL',180,true,false,true,110),
('BUSINESS_FULL','Business Full','Empresa completa: comercial, estoque, OS, manutenÃ§Ã£o, indÃºstria e financeiro.','MENSAL',0,'BRL','MENSAL',250,false,false,true,120)
on conflict (codigo) do update set nome=excluded.nome, descricao=excluded.descricao, limite_usuarios=excluded.limite_usuarios, publico=excluded.publico, destaque=excluded.destaque, ativo=true, updated_at=now();
...
('Gerente Industrial','Gerencia produÃ§Ã£o, chÃ£o de fÃ¡brica, custos e indicadores.','GERENTE_INDUSTRIAL',true),
('PCP','Planeja e libera ordens de produÃ§Ã£o.','PCP',true),
('Operador de ProduÃ§Ã£o','Realiza apontamentos de chÃ£o de fÃ¡brica.','OPERADOR_PRODUCAO',true),
('Qualidade','Executa inspeÃ§Ãµes e liberaÃ§Ãµes de qualidade.','QUALIDADE',true),
('ManutenÃ§Ã£o','Atua em paradas e OS de manutenÃ§Ã£o.','MANUTENCAO',true)
on conflict do nothing;
...
('industria','fichas','visualizar','industria.fichas.visualizar','Visualizar fichas tÃ©cnicas',true),
('industria','fichas','criar','industria.fichas.criar','Criar fichas tÃ©cnicas',true),
('industria','fichas','editar','industria.fichas.editar','Editar fichas tÃ©cnicas',true),
...
('industria','ordens','visualizar','industria.ordens.visualizar','Visualizar ordens de produÃ§Ã£o',true),
('industria','ordens','criar','industria.ordens.criar','Criar ordens de produÃ§Ã£o',true),
('industria','ordens','liberar','industria.ordens.liberar','Liberar ordens de produÃ§Ã£o',true),
('industria','ordens','iniciar','industria.ordens.iniciar','Iniciar ordens de produÃ§Ã£o',true),
('industria','ordens','concluir','industria.ordens.concluir','Concluir ordens de produÃ§Ã£o',true),
...
('industria','producao','registrar','industria.producao.registrar','Registrar produÃ§Ã£o acabada',true),
...
('industria','chao_fabrica','acessar','industria.chao_fabrica.acessar','Acessar chÃ£o de fÃ¡brica',true)
on conflict (modulo,recurso,acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;
...
('financeiro_empresarial','Financeiro Empresarial','Plano de contas, centros de custo, contas, baixas, movimentos, fluxo de caixa e conciliaÃ§Ã£o.','GestÃ£o empresarial',45,'/Financeiro/Dashboard','bi-bank',true),
('financeiro_publico','Financeiro PÃºblico','Base futura para SIAFIC e financeiro pÃºblico integrado.','GestÃ£o pÃºblica',46,'/Financeiro/Dashboard','bi-building-lock',true)
...
-- SIGOV - EvoluÃ§Ã£o PÃ³s-Build 08: TributÃ¡rio AvanÃ§ado e Fiscal Integrado
...
insert into sigov.permissao (modulo,recurso,acao,chave,descricao,ativo) values
('tributario','dashboard','visualizar','tributario.dashboard.visualizar','Visualizar dashboard tributÃ¡rio avanÃ§ado',true),
...
('tributario','divida_ativa','visualizar','tributario.divida_ativa.visualizar','Visualizar dÃ­vida ativa',true),
('tributario','divida_ativa','editar','tributario.divida_ativa.editar','Editar dÃ­vida ativa',true),
('tributario','parcelamento','visualizar','tributario.parcelamento.visualizar','Visualizar parcelamentos tributÃ¡rios',true),
('tributario','parcelamento','editar','tributario.parcelamento.editar','Editar parcelamentos tributÃ¡rios',true),
('tributario','arrecadacao','visualizar','tributario.arrecadacao.visualizar','Visualizar arrecadaÃ§Ã£o tributÃ¡ria',true),
('tributario','arrecadacao','registrar','tributario.arrecadacao.registrar','Registrar arrecadaÃ§Ã£o tributÃ¡ria',true),
...
('tributario','livro_eletronico','visualizar','tributario.livro_eletronico.visualizar','Visualizar livro eletrÃ´nico tributÃ¡rio',true),
('tributario','livro_eletronico','gerar','tributario.livro_eletronico.gerar','Gerar livro eletrÃ´nico tributÃ¡rio',true)
on conflict (modulo,recurso,acao) do update set chave=excluded.chave, descricao=excluded.descricao, ativo=true;

insert into sigov.perfil_acesso (nome, descricao, codigo_externo, ativo) values
('TributÃ¡rio Admin','Administra IPTU, ISS, taxas, DAM, dÃ­vida ativa, NFS-e simulada e livro eletrÃ´nico.','TRIBUTARIO_ADMIN',true),
('Fiscal TributÃ¡rio','Opera lanÃ§amentos, arrecadaÃ§Ã£o, parcelamentos e relatÃ³rios fiscais.','FISCAL_TRIBUTARIO',true),
('Consulta TributÃ¡ria','Consulta dashboard, livro eletrÃ´nico e relatÃ³rios fiscais.','CONSULTA_TRIBUTARIA',true)
on conflict do nothing;
...
insert into sigov.tenant_modulo_pacote (codigo, nome, descricao, modulos_json) values
('GOV_TRIBUTARIO_PLUS','Gov TributÃ¡rio Plus','Pacote fiscal municipal com TributÃ¡rio AvanÃ§ado, Financeiro PÃºblico e LGPD.', '["tributario","financeiro_publico","auditoria-lgpd"]'::jsonb)
...
from sigov.tenant t
cross join (values
    ('IPTU','Imposto Predial e Territorial Urbano','IPTU',1.0000,'CÃ³digo TributÃ¡rio Municipal - IPTU','IMOBILIARIO'),
    ('ISS','Imposto Sobre ServiÃ§os','ISS',2.0000,'CÃ³digo TributÃ¡rio Municipal - ISS','SERVICO'),
    ('TAXA_COLETA','Taxa de Coleta de ResÃ­duos','TAXA',0.0000,'CÃ³digo TributÃ¡rio Municipal - Taxas','SERVICO')
...
-- SIGOV PLUS - PÃ³s-Build 09: GED/OCR, assinatura digital simulada e automaÃ§Ã£o documental.
...
('ged','tramitacao','tramitar','ged.tramitar','Tramitar documentos e protocolos eletrÃ´nicos.',true),
('contrato','contrato','visualizar','contrato.visualizar','Visualizar contratos jurÃ­dicos e comerciais.',true),
...
from sigov.tenant t
cross join (values
    ('CONTRATO','Contrato','Instrumentos jurÃ­dicos, comerciais e administrativos.',true,true,'["numero","vigencia","contratado"]'),
    ('DOCUMENTO_FISCAL','Documento fiscal','Notas, DAM, comprovantes e livros fiscais vinculados ao TributÃ¡rio.',false,true,'["competencia","valor","origem"]'),
    ('CHECKLIST_PRODUCAO','Checklist de produÃ§Ã£o','Checklist, qualidade e evidÃªncias de ordens industriais.',false,true,'["ordem","produto","etapa"]'),
    ('PROTOCOLO','Protocolo eletrÃ´nico','Documentos recebidos via protocolo e atendimento.',false,true,'["numero_protocolo","canal"]')
...
from sigov.tenant t
cross join (values
    ('TPL_CONTRATO_PADRAO','Contrato padrÃ£o','CONTRATO','Contrato nÂº {{numero}} firmado com {{contratado}} no valor de {{valor_total}}.','["numero","contratado","valor_total"]'),
    ...
    ('TPL_CHECKLIST_OP','Checklist de ordem de produÃ§Ã£o','CHECKLIST_PRODUCAO','Checklist da ordem {{ordem}} para produto {{produto}} na etapa {{etapa}}.','["ordem","produto","etapa"]')
...
select t.id, 'WF_GED_BASICO', 'Workflow bÃ¡sico GED/OCR/Assinatura', 'RECEBIMENTO', 'ATIVO', '{"etapas":["RECEBIMENTO","OCR","INDEXACAO","TRAMITACAO","ASSINATURA","ARQUIVAMENTO"],"assinatura":"SIMULADA"}'::jsonb
from sigov.tenant t
where t.slug in ('plataforma-global','prefeitura-demo','tenant-demo')
...
insert into sigov.tenant_modulo_pacote (codigo, nome, descricao, modulos_json) values
('GED_AUTOMACAO_PLUS','GED AutomaÃ§Ã£o Plus','GED completo com OCR simulado, contratos, protocolos, workflow, tramitaÃ§Ã£o e assinatura digital simulada.', '["ged","ocr","contrato","fluxo","processos","integracoes","auditoria-lgpd"]'::jsonb)
on conflict (codigo) do update set nome=excluded.nome, descricao=excluded.descricao, modulos_json=excluded.modulos_json;
...
-- PÃ³s-Build 11: IA, automaÃ§Ã£o inteligente, assistentes operacionais e anÃ¡lise preditiva
...
('INTERNO','Provider interno heurÃ­stico','INTERNO',null,true),
('LOCAL','Provider local configurÃ¡vel','LOCAL',null,true),
('OPENAI','OpenAI configurÃ¡vel','OPENAI',null,false),
('AZURE_OPENAI','Azure OpenAI configurÃ¡vel','AZURE_OPENAI',null,false)
...
('ASSISTENTE_TRIBUTARIO','Assistente TributÃ¡rio','Apoio a anÃ¡lises fiscais e arrecadaÃ§Ã£o.','TRIBUTARIO',true),
('ASSISTENTE_FINANCEIRO','Assistente Financeiro','Apoio a fluxo de caixa, cobranÃ§a e inadimplÃªncia.','FINANCEIRO',true),
('ASSISTENTE_GED','Assistente GED','Resumo, classificaÃ§Ã£o e extraÃ§Ã£o documental.','GED',true),
('ASSISTENTE_COMERCIAL','Assistente Comercial','Apoio a clientes, leads e oportunidades.','COMERCIAL',true),
('ASSISTENTE_INDUSTRIA','Assistente IndÃºstria','Apoio ao chÃ£o de fÃ¡brica e produÃ§Ã£o.','INDUSTRIA',true),
('ASSISTENTE_OS','Assistente OS','Apoio a ordens de serviÃ§o.','OS',true),
('ASSISTENTE_JURIDICO','Assistente JurÃ­dico','GeraÃ§Ã£o assistida de pareceres.','JURIDICO',true),
...
(null,'resumo_processo','Resumo de processo','protocolo','RESUMO','Resuma o processo e sugira prÃ³ximos passos sem executar aÃ§Ãµes crÃ­ticas.',false,true),
(null,'resumo_os','Resumo de OS','ordem_servico','RESUMO','Resuma o histÃ³rico da OS e a prÃ³xima aÃ§Ã£o segura.',false,true),
(null,'resumo_contrato','Resumo de contrato','contrato','RESUMO','Resuma contrato, vigÃªncia, clÃ¡usulas e riscos.',false,true),
(null,'classificar_documento','Classificar documento','ged','CLASSIFICACAO','Classifique por palavras-chave e indique confianÃ§a.',false,true),
(null,'extrair_dados_documento','Extrair dados de documento','ged','EXTRACAO','Extraia nÃºmero, datas, valores e partes, mascarando dados sensÃ­veis.',false,true),
(null,'gerar_relatorio_financeiro','Gerar relatÃ³rio financeiro','financeiro_empresarial','RELATORIO','Gere relatÃ³rio textual financeiro com riscos e oportunidades.',false,true),
(null,'gerar_parecer_juridico','Gerar parecer jurÃ­dico','juridico','PARECER','Rascunhe parecer para revisÃ£o humana obrigatÃ³ria.',true,true),
(null,'gerar_despacho_protocolo','Gerar despacho de protocolo','protocolo','DESPACHO','Rascunhe despacho administrativo para revisÃ£o humana.',true,true),
(null,'sugerir_acoes_cobranca','Sugerir aÃ§Ãµes de cobranÃ§a','financeiro_empresarial','SUGESTAO','Sugira aÃ§Ãµes de cobranÃ§a sem baixa financeira automÃ¡tica.',true,true),
(null,'prever_inadimplencia','Prever inadimplÃªncia','financeiro_empresarial','ANALISE','Estime risco de inadimplÃªncia por regras.',false,true),
(null,'prever_atraso_os','Prever atraso OS','ordem_servico','ANALISE','Estime risco de atraso em OS.',false,true),
(null,'prever_ruptura_estoque','Prever ruptura estoque','estoque_compras','ANALISE','Estime risco de ruptura por saldo e estoque mÃ­nimo.',false,true),
(null,'sugerir_reposicao_estoque','Sugerir reposiÃ§Ã£o','estoque_compras','SUGESTAO','Sugira compra/reposiÃ§Ã£o com confirmaÃ§Ã£o humana.',true,true),
(null,'analisar_fluxo_caixa','Analisar fluxo de caixa','financeiro_empresarial','ANALISE','Analise tendÃªncias e alertas no fluxo de caixa.',false,true),
(null,'analisar_producao_atrasada','Analisar produÃ§Ã£o atrasada','industria_producao','ANALISE','Identifique risco e causa provÃ¡vel de atraso produtivo.',false,true)
...
('inadimplencia_cliente','InadimplÃªncia de cliente','Modelo heurÃ­stico inicial.','financeiro_empresarial','INADIMPLENCIA',true),
('atraso_ordem_servico','Atraso em ordem de serviÃ§o','Modelo heurÃ­stico inicial.','ordem_servico','ATRASO_OS',true),
('ruptura_estoque','Ruptura de estoque','Modelo heurÃ­stico inicial.','estoque_compras','ESTOQUE_RUPTURA',true),
('risco_contrato','Risco contratual','Modelo heurÃ­stico inicial.','contrato','CONTRATO_RISCO',true),
('atraso_producao','Atraso de produÃ§Ã£o','Modelo heurÃ­stico inicial.','industria_producao','PRODUCAO_ATRASO',true),
('queda_arrecadacao','Queda de arrecadaÃ§Ã£o','Modelo heurÃ­stico inicial.','tributario','ARRECADACAO',true),
('churn_saas','Churn SaaS','Modelo heurÃ­stico inicial.','operacao-saas','CHURN',true)
...
insert into sigov.modulo_saas(codigo,nome,descricao,categoria,ordem,rota_base,icone,ativo) values
('ia_assistente','IA Assistente','Assistentes inteligentes por mÃ³dulo, seguros e auditÃ¡veis.','InteligÃªncia',900,'/IA/Assistente','bi-robot',true),
('ia_documental','IA Documental','Resumo, classificaÃ§Ã£o e extraÃ§Ã£o estruturada de documentos.','InteligÃªncia',901,'/IA/Documental','bi-file-earmark-text',true),
('ia_relatorios','IA RelatÃ³rios','GeraÃ§Ã£o assistida de relatÃ³rios textuais.','InteligÃªncia',902,'/IA/Relatorios','bi-bar-chart-line',true),
('ia_automacoes','IA AutomaÃ§Ãµes','SugestÃµes, alertas e workflows com confirmaÃ§Ã£o humana.','InteligÃªncia',903,'/IA/Automacoes','bi-diagram-3',true),
('ia_predicoes','IA PrediÃ§Ãµes','AnÃ¡lise preditiva inicial baseada em regras.','InteligÃªncia',904,'/IA/Predicoes','bi-graph-up-arrow',true)
...
('ia_1000_interacoes','IA 1.000 interaÃ§Ãµes','Pacote mensal adicional de 1.000 interaÃ§Ãµes de IA.','IA_CONSUMO','ia_assistente',null,'MENSAL'),
('ia_10000_interacoes','IA 10.000 interaÃ§Ãµes','Pacote mensal adicional de 10.000 interaÃ§Ãµes de IA.','IA_CONSUMO','ia_assistente',null,'MENSAL'),
('ia_ocr_avancado','IA OCR avanÃ§ado','Capacidades avanÃ§adas de OCR e extraÃ§Ã£o.','IA_RECURSO','ia_documental',null,'MENSAL'),
('ia_relatorios_avancados','IA relatÃ³rios avanÃ§ados','RelatÃ³rios assistidos avanÃ§ados.','IA_RECURSO','ia_relatorios',null,'MENSAL'),
('ia_automacoes_avancadas','IA automaÃ§Ãµes avanÃ§adas','Workflows inteligentes avanÃ§ados.','IA_RECURSO','ia_automacoes',null,'MENSAL')
on conflict(codigo) do update set nome=excluded.nome,descricao=excluded.descricao,tipo_addon=excluded.tipo_addon,modulo_codigo=excluded.modulo_codigo;

insert into sigov.tenant_modulo_pacote(codigo,nome,descricao,modulos_json) values
('AI_STARTER','AI Starter','Assistente e relatÃ³rios por IA.','["ia_assistente","ia_relatorios"]'::jsonb),
...
('GOV_FULL_AI','Gov Full AI','Governo com IA documental, relatÃ³rios e automaÃ§Ãµes.','["tributario","protocolo","ged","contratos","financeiro_publico","ia_assistente","ia_documental","ia_relatorios","ia_automacoes"]'::jsonb)
on conflict(codigo) do update set nome=excluded.nome,descricao=excluded.descricao,modulos_json=excluded.modulos_json;
...
('ia','execucoes','visualizar','ia.execucoes.visualizar','Visualizar execuÃ§Ãµes IA',true),
('ia','sugestoes','visualizar','ia.sugestoes.visualizar','Visualizar sugestÃµes IA',true),
('ia','sugestoes','aprovar','ia.sugestoes.aprovar','Aprovar sugestÃµes IA',true),
('ia','sugestoes','aplicar','ia.sugestoes.aplicar','Aplicar sugestÃµes IA',true),
('ia','sugestoes','rejeitar','ia.sugestoes.rejeitar','Rejeitar sugestÃµes IA',true),
...
('ia','relatorios','gerar','ia.relatorios.gerar','Gerar relatÃ³rios por IA',true),
('ia','automacoes','visualizar','ia.automacoes.visualizar','Visualizar automaÃ§Ãµes IA',true),
('ia','automacoes','criar','ia.automacoes.criar','Criar automaÃ§Ãµes IA',true),
('ia','automacoes','editar','ia.automacoes.editar','Editar automaÃ§Ãµes IA',true),
('ia','automacoes','executar','ia.automacoes.executar','Executar automaÃ§Ãµes IA',true),
...
('ia','predicoes','visualizar','ia.predicoes.visualizar','Visualizar prediÃ§Ãµes IA',true),
('ia','predicoes','executar','ia.predicoes.executar','Executar prediÃ§Ãµes IA',true),
('ia','configuracao','visualizar','ia.configuracao.visualizar','Visualizar configuraÃ§Ã£o IA',true),
('ia','configuracao','editar','ia.configuracao.editar','Editar configuraÃ§Ã£o IA',true),
...
-- SIGOV PÃ³s-Build 12: Mobile/PWA, Campo, Offline First, Georreferenciamento e SincronizaÃ§Ã£o
create schema if not exists sigov;
...
create table if not exists sigov.campo_localizacao (
  ...
  regra_operacional text not null default 'Coleta opcional conforme permissÃ£o e finalidade operacional.',
  capturado_at timestamptz not null default now(),
  created_at timestamptz not null default now()
);
...
(1,'checklist_os_tecnica','Checklist OS TÃ©cnica','ordem_servico','ORDEM_SERVICO'),
(1,'checklist_visita_social','Checklist Visita Social','social','VISITA'),
(1,'checklist_visita_acs','Checklist Visita ACS','saude','VISITA'),
(1,'checklist_leitura_hidrometro','Checklist Leitura de HidrÃ´metro','saneamento','LEITURA'),
(1,'checklist_vistoria_tributaria','Checklist Vistoria TributÃ¡ria','tributario','FISCALIZACAO'),
(1,'checklist_visita_agro','Checklist Visita Agro','agro','VISITA'),
(1,'checklist_manutencao_industrial','Checklist ManutenÃ§Ã£o Industrial','manutencao_industrial','MANUTENCAO'),
...
join (values
('checklist_os_tecnica','seguranca','Checklist de seguranÃ§a preenchido?','SIM_NAO',true,1),
...
('checklist_visita_acs','domicilio','DomicÃ­lio visitado?','SIM_NAO',true,1),
...
(1,'Mobile/PWA habilitado','Ambiente mobile, offline sync e campo operacional disponÃ­veis para demonstraÃ§Ã£o.','INFO',true)
on conflict do nothing;

-- Tokens/seeds pesquisÃ¡veis pelo script completo/testes estÃ¡ticos: mobile_usuarios_extra, storage_fotos_campo, geolocalizacao_avancada, sincronizacao_offline_avancada,
