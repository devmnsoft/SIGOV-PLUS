...
## Operação em campo
Atividades, visitas, checklists, evidências, assinatura, localização, rotas, formulários e notificações são expostos por `/api/campo/*` e telas administrativas `/Campo/*`.

## Integrações por módulo
OS técnica, saneamento/leitura, saúde/ACS, agro, social, tributário, manutenção industrial e entrega comercial usam o mesmo núcleo `campo_atividade` com `modulo_codigo`, origem e origem_id.
