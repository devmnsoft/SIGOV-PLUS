...
Próximo item exato: validar sintaxe do aplicador canônico com `pwsh -NoProfile -File scripts/apply-migrations-manifest.ps1 -ValidateOnly`; em PG16, provar banco vazio/reaplicação, rejeição de versão e checksum desconhecidos, aceitação de `knownChecksums` com pós-condição e upgrade legado formal. O Bash somente poderá voltar a aplicar migrations quando implementar o mesmo ledger, pós-condições, probes, compatibilidade e transação do aplicador canônico. Depois obter evidências de revogação/tenant suspenso/acesso cruzado API; só então Gate B (SaaS). Gates C–D e GED não avançam.
