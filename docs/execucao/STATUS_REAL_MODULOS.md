...
Atualização da jornada de processos em 2026-09-22: movimentação, conclusão e cancelamento passaram a proteger estado concorrente e a gravar estado/histórico atomicamente; a listagem aplica os filtros reais de entidade, exercício e período, e o detalhe recebeu ações com justificativa, prazo explícito, ajuda contextual e codificação de conteúdo dinâmico. A classificação permanece **PARCIAL**, pois build, PostgreSQL 16, autenticação, dois tenants, recebimento formal, anexos, central e prazos parametrizados não puderam ser validados. Consulte `docs/execucao/AUDITORIA_JORNADA_PROCESSOS_2026-09-22.md`.
