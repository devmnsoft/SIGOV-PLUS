...
public sealed class OrdemServicoApplicationService(IOrdemServicoRepository repository) : IOrdemServicoApplicationService
{
    private static void Validar(OrdemServicoContext c) { if (c.TenantId == Guid.Empty || c.UsuarioId == Guid.Empty) throw new UnauthorizedAccessException("Tenant e usuário são obrigatórios."); }
    ...
    public Task ConsumirPecaAsync(OrdemServicoContext c, Guid id, ConsumirPecaRequest r, string key, CancellationToken ct) { Validar(c); Chave(key); if (r.Quantidade <= 0) throw new ArgumentException("Quantidade deve ser positiva."); return repository.ConsumirAsync(c.TenantId, c.UsuarioId, id, r, key, c.CorrelationId, ct); }
    public Task DevolverPecaAsync(OrdemServicoContext c, Guid id, DevolverPecaRequest r, string key, CancellationToken ct) { Validar(c); Chave(key); if (r.Quantidade <= 0) throw new ArgumentException("Quantidade deve ser positiva."); return repository.DevolverAsync(c.TenantId, c.UsuarioId, id, r, key, c.CorrelationId, ct); }
    public Task<IReadOnlyList<OrdemServicoEvidenciaDto>> EvidenciasAsync(OrdemServicoContext c, Guid id, CancellationToken ct) { Validar(c); return repository.EvidenciasAsync(c.TenantId, id, ct); }
    public Task AdicionarEvidenciaAsync(OrdemServicoContext c, Guid id, AdicionarEvidenciaRequest r, string key, CancellationToken ct) { Validar(c); Chave(key); if (r.DocumentoGedId == Guid.Empty || string.IsNullOrWhiteSpace(r.Nome)) throw new ArgumentException("Documento GED e nome são obrigatórios."); return repository.AdicionarEvidenciaAsync(c.TenantId, c.UsuarioId, id, r, key, c.CorrelationId, ct); }
    public Task<OrdemServicoAceiteDto?> ObterAceiteAsync(OrdemServicoContext c, Guid id, CancellationToken ct) { Validar(c); return repository.ObterAceiteAsync(c.TenantId, id, ct); }
    public Task RegistrarAceiteAsync(OrdemServicoContext c, Guid id, RegistrarAceiteClienteRequest r, string key, CancellationToken ct) { Validar(c); Chave(key); if (!r.Confirmado || string.IsNullOrWhiteSpace(r.Nome)) throw new ArgumentException("A confirmação e o nome do responsável são obrigatórios."); return repository.RegistrarAceiteAsync(c.TenantId, c.UsuarioId, id, r, key, c.CorrelationId, ct); }
    public Task<OrdemServicoCustoDto> ObterCustosAsync(OrdemServicoContext c, Guid id, CancellationToken ct) { Validar(c); return repository.ObterCustosAsync(c.TenantId, id, ct); }
    public Task<OrdemServicoDashboardDto> DashboardAsync(OrdemServicoContext c, CancellationToken ct) { Validar(c); return repository.DashboardAsync(c.TenantId, ct); }
}
