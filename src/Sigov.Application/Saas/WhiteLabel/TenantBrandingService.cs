...
public sealed class TenantBrandingService : ITenantBrandingService
{
    ...
    public async Task<TenantBrandingResponse> GetAsync(long tenantId, CancellationToken cancellationToken) =>
        await _repository.GetAsync(tenantId, cancellationToken).ConfigureAwait(false) ?? new TenantBrandingResponse(0, tenantId, "sigov", null, "#0d6efd", "#6c757d", "#198754", "SIGOV", null, null, false, await _repository.PlanoPermiteWhiteLabelAsync(tenantId, cancellationToken).ConfigureAwait(false));
