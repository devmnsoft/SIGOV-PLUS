...
public sealed class TenantBrandingService : ITenantBrandingService
{
    ...
    public async Task<TenantBrandingResponse> GetAsync(long tenantId, CancellationToken cancellationToken) =>
        ...
        ?? new TenantBrandingResponse(0, tenantId, "sigov", null, "#0d6efd", "#6c757d", "#198754", "SIGOV", null, null, false, await _repository.PlanoPermiteWhiteLabelAsync(tenantId, cancellationToken).ConfigureAwait(false));
    ...
    public async Task<Result<TenantBrandingResponse>> UpdateAsync(long tenantId, TenantBrandingUpdateRequest request, long usuarioId, CancellationToken cancellationToken)
    {
        ...
        if (validation.IsFailure) return Result<TenantBrandingResponse>.Failure(validation.Error ?? "Branding invalido.");
        ...
        await _repository.InsertEventoAsync(tenantId, "TenantBrandingAtualizado", "saas_tenant_branding", updated.Id, new { updated.TenantId, updated.WhiteLabelAtivo, updated.LogoUrl, updated.LogoWidthPx, updated.LogoHeightPx, updated.LogoFit }, correlationId, cancellationToken).ConfigureAwait(false);
