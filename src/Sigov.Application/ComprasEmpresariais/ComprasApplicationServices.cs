...
public sealed class RequisicaoCompraApplicationService(IRequisicaoCompraRepository repository):IRequisicaoCompraApplicationService
{
 ...
 public Task AtualizarAsync(ComprasContext c,Guid id,AtualizarRequisicaoRequest r,CancellationToken ct){ComprasGuard.Context(c);if(id==Guid.Empty||r.Version<=0)throw new ArgumentException("Requisição ou versão inválida.");if(r.Itens.Count==0)throw new ArgumentException("Inclua ao menos um item.");if(r.Itens.Any(x=>x.Quantidade<=0||x.ValorEstimado<0))throw new ArgumentException("Quantidade e valor dos itens são inválidos.");return repository.AtualizarAsync(c,id,r,ct);}
