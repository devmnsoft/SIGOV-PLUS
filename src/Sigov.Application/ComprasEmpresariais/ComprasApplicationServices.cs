...
public sealed class DevolucaoCompraApplicationService(IDevolucaoCompraRepository repository):IDevolucaoCompraApplicationService
{
 ...
 public Task<DevolucaoComandoResultado> EditarAsync(ComprasContext c,long id,EditarDevolucaoRequest r,CancellationToken ct){Validate(c,r.IdempotencyKey,r.Motivo,r.OrigemFisica,r.Destino,r.Itens);Version(id,r.Version);if(r.ResponsavelId==Guid.Empty)throw new ArgumentException("Selecione o responsável.");return repository.EditarAsync(c,id,r,ct);}
 public Task<DevolucaoComandoResultado> ExpedirAsync(ComprasContext c,long id,ExpedirDevolucaoRequest r,CancellationToken ct){Command(c,id,r.Version,r.IdempotencyKey);if(r.SaidaEm==default||r.Modalidade is not("TRANSPORTADORA" or "ENTREGA_DIRETA"))throw new ArgumentException("Informe data e modalidade da saída.");if(r.SaidaEm>DateTimeOffset.UtcNow.AddMinutes(5))throw new ArgumentException("A confirmação de saída física não pode registrar data futura.");if(r.Modalidade=="TRANSPORTADORA"&&string.IsNullOrWhiteSpace(r.Transportadora))throw new ArgumentException("Informe a transportadora para esta modalidade.");return repository.ExpedirAsync(c,id,r,ct);}
 public Task<DevolucaoComandoResultado> EntregarAsync(ComprasContext c,long id,EntregarDevolucaoRequest r,CancellationToken ct){Command(c,id,r.Version,r.IdempotencyKey);if(r.EntregueEm==default||string.IsNullOrWhiteSpace(r.RecebedorOuReferencia)||string.IsNullOrWhiteSpace(r.DocumentoProtocolo))throw new ArgumentException("Data, recebedor/referência e protocolo são obrigatórios.");if(r.EntregueEm>DateTimeOffset.UtcNow.AddMinutes(5))throw new ArgumentException("A confirmação de entrega física não pode registrar data futura.");return repository.EntregarAsync(c,id,r,ct);}
