...
returning id",
    ...
    public async Task<RematriculaSimulacaoResponse> SimularRematriculaAsync(long tenantId, long entidadeId, RematriculaFiltro filtro, CancellationToken ct)
    {
        const string contextSql = @"select exists(select 1 from sigov.escola where tenant_id=@TenantId and entidade_id=@EntidadeId and id=@EscolaId and situacao='ATIVA' and not is_deleted)
 and exists(select 1 from sigov.ano_letivo where tenant_id=@TenantId and entidade_id=@EntidadeId and id=@Origem and escola_id=@EscolaId and not is_deleted)
 and exists(select 1 from sigov.ano_letivo where tenant_id=@TenantId and entidade_id=@EntidadeId and id=@Destino and escola_id=@EscolaId and status in ('PLANEJADO','ABERTO','ATIVO') and not is_deleted)";
        const string sql = @"select m.id as ""MatriculaOrigemId"",m.aluno_id as ""AlunoId"",a.codigo_aluno as ""AlunoCodigo"",m.numero_matricula as ""MatriculaOrigem"",
 t.id as ""TurmaOrigemId"",t.nome as ""TurmaOrigem"",r.resultado as ""Resultado"",r.versao as ""ResultadoVersao"",
 td.id as ""TurmaDestinoId"",td.nome as ""TurmaDestino"",case when td.id is null then null else td.capacidade-td.vagas_ocupadas end as ""VagasDisponiveis"",
 md.id as ""MatriculaDestinoId"",
 case when md.id is not null then 'JA_REMATRICULADO' when r.id is null or r.status<>'PUBLICADO' then 'DEPENDENTE_ANALISE'
      when r.resultado not in ('APROVADO','REPROVADO','CONCLUIDO') then 'IMPEDIDO'
      when td.id is null then 'SEM_OFERTA_COMPATIVEL' when td.status not in ('ABERTA','PLANEJADA') or not td.ativo then 'IMPEDIDO'
      when td.vagas_ocupadas>=td.capacidade then 'IMPEDIDO' else 'ELEGIVEL' end as ""Situacao"",
 case when md.id is not null then 'Matrícula de destino já existe e será preservada.' when r.id is null then 'Resultado final não publicado; ausência não equivale a aprovação.'
      when r.status<>'PUBLICADO' then 'Resultado final ainda não está publicado.' when r.resultado not in ('APROVADO','REPROVADO','CONCLUIDO') then 'Resultado não autoriza decisão automática.'
      when td.id is null then 'Destino não definido; selecione uma oferta autorizada.' when td.status not in ('ABERTA','PLANEJADA') or not td.ativo then 'Oferta de destino inativa.'
      when td.vagas_ocupadas>=td.capacidade then 'Turma de destino sem vaga.' else 'Validações concluídas; a vaga só será consumida na confirmação.' end as ""Motivo"",
 md5(concat_ws('|',m.id,r.id,r.versao,r.resultado,r.status,coalesce(td.id,0),coalesce(td.vagas_ocupadas,-1),coalesce(td.status,''),coalesce(td.updated_at,td.created_at),coalesce(md.id,0))) as ""TokenConferencia""
from sigov.matricula m join sigov.aluno a on a.id=m.aluno_id and a.tenant_id=m.tenant_id and a.entidade_id=m.entidade_id and not a.is_deleted
join sigov.turma t on t.id=m.turma_id and t.tenant_id=m.tenant_id and t.entidade_id=m.entidade_id and not t.is_deleted
left join lateral(select x.* from sigov.educacao_resultado_final x where x.tenant_id=m.tenant_id and x.entidade_id=m.entidade_id and x.matricula_id=m.id and not x.is_deleted order by x.versao desc limit 1) r on true
left join sigov.educacao_progressao_config pc on pc.tenant_id=m.tenant_id and pc.entidade_id=m.entidade_id and pc.serie_origem_id=t.serie_ano_id and pc.resultado=r.resultado and pc.ativo and not pc.is_deleted
left join sigov.turma td on td.tenant_id=m.tenant_id and td.entidade_id=m.entidade_id and td.escola_id=@EscolaId and td.ano_letivo_id=@Destino and not td.is_deleted
 and ((@TurmaDestino is not null and td.id=@TurmaDestino) or (@TurmaDestino is null and td.serie_ano_id=pc.serie_destino_id))
left join sigov.matricula md on md.tenant_id=m.tenant_id and md.entidade_id=m.entidade_id and md.aluno_id=m.aluno_id and md.ano_letivo_id=@Destino and md.origem_matricula_id=m.id and md.status<>'CANCELADA' and not md.is_deleted
where m.tenant_id=@TenantId and m.entidade_id=@EntidadeId and m.escola_id=@EscolaId and m.ano_letivo_id=@Origem and m.status in ('ATIVA','CONFIRMADA','CONCLUIDA') and not m.is_deleted
 and (@TurmaOrigem is null or m.turma_id=@TurmaOrigem) order by a.codigo_aluno,m.id";
        var p = new { TenantId=tenantId, EntidadeId=entidadeId, filtro.EscolaId, Origem=filtro.AnoLetivoOrigemId, Destino=filtro.AnoLetivoDestinoId, TurmaOrigem=filtro.TurmaOrigemId, TurmaDestino=filtro.TurmaDestinoId };
        using var connection = _context.CreateConnection();
        if (!await connection.ExecuteScalarAsync<bool>(new CommandDefinition(contextSql, p, cancellationToken:ct)).ConfigureAwait(false)) throw new InvalidOperationException("Origem ou oferta de destino inexistente, inativa ou fora do contexto autorizado.");
        var items=(await connection.QueryAsync<RematriculaCandidatoResponse>(new CommandDefinition(sql,p,cancellationToken:ct)).ConfigureAwait(false)).AsList();
        if (!string.IsNullOrWhiteSpace(filtro.Situacao)) items=items.Where(x=>x.Situacao.Equals(filtro.Situacao,StringComparison.OrdinalIgnoreCase)).ToList();
        return new(filtro,items,items.Count(x=>x.Situacao=="ELEGIVEL"),items.Count(x=>x.Situacao is "DEPENDENTE_ANALISE" or "SEM_OFERTA_COMPATIVEL"),items.Count(x=>x.Situacao=="JA_REMATRICULADO"));
    }

    public async Task CancelarMatriculaAsync(long tenantId,long entidadeId,long matriculaId,string motivo,long usuarioId,CancellationToken ct)
    {
        using var c=(NpgsqlConnection)_context.CreateConnection();await c.OpenAsync(ct).ConfigureAwait(false);await using var tx=await c.BeginTransactionAsync(ct).ConfigureAwait(false);
        var m=await c.QuerySingleOrDefaultAsync<(string Status,long? TurmaId,long AlunoId)>(new CommandDefinition("select status as Status,turma_id as TurmaId,aluno_id as AlunoId from sigov.matricula where id=@Id and tenant_id=@TenantId and entidade_id=@EntidadeId and not is_deleted for update",new{Id=matriculaId,TenantId=tenantId,EntidadeId=entidadeId},tx,cancellationToken:ct)).ConfigureAwait(false);
        if(string.IsNullOrEmpty(m.Status))throw new InvalidOperationException("Matrícula não encontrada no contexto autorizado.");if(m.Status=="CANCELADA"){await tx.CommitAsync(ct).ConfigureAwait(false);return;}
        var deps=await c.QuerySingleAsync<(long Frequencias,long Notas,long Documentos)>(new CommandDefinition(@"select
 (select count(*) from sigov.diario_frequencia where tenant_id=@TenantId and entidade_id=@EntidadeId and aluno_id=@Aluno and (@Turma is null or turma_id=@Turma) and not is_deleted) as Frequencias,
 (select count(*) from sigov.nota n join sigov.avaliacao a on a.id=n.avaliacao_id and a.tenant_id=n.tenant_id and a.entidade_id=n.entidade_id where n.tenant_id=@TenantId and n.entidade_id=@EntidadeId and n.aluno_id=@Aluno and (@Turma is null or a.turma_id=@Turma) and not n.is_deleted) as Notas,
 (select count(*) from sigov.educacao_documento_escolar where tenant_id=@TenantId and matricula_id=@Id and not is_deleted) as Documentos",new{Id=matriculaId,TenantId=tenantId,EntidadeId=entidadeId,Aluno=m.AlunoId,Turma=m.TurmaId},tx,cancellationToken:ct)).ConfigureAwait(false);
        if(deps.Frequencias+deps.Notas+deps.Documentos>0)throw new InvalidOperationException($"Cancelamento impedido: {deps.Frequencias} frequências, {deps.Notas} notas e {deps.Documentos} documentos dependem da matrícula. Use o procedimento autorizado de retificação antes de cancelar.");
        await c.ExecuteAsync(new CommandDefinition("update sigov.matricula set status='CANCELADA',observacao=concat_ws(E'\\n',observacao,@Motivo),updated_at=now(),updated_by=@Usuario where id=@Id; update sigov.turma set vagas_ocupadas=greatest(vagas_ocupadas-1,0),updated_at=now(),updated_by=@Usuario where id=@Turma",new{Id=matriculaId,Turma=m.TurmaId,Motivo=$"Cancelamento: {motivo}",Usuario=usuarioId},tx,cancellationToken:ct)).ConfigureAwait(false);
        await RegistrarEventoAsync(c,tx,tenantId,entidadeId,"MatriculaCancelada","matricula",matriculaId,new{motivo},usuarioId,ct).ConfigureAwait(false);await tx.CommitAsync(ct).ConfigureAwait(false);
    }

    public async Task<RematriculaConfirmacaoResponse> ConfirmarRematriculaAsync(long tenantId, long entidadeId, long? exercicioId, RematriculaConfirmacaoRequest request, long usuarioId, CancellationToken ct)
    {
        var payload=JsonSerializer.Serialize(request,JsonOptions); var hash=Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(Encoding.UTF8.GetBytes(payload))).ToLowerInvariant();
        using var connection=(NpgsqlConnection)_context.CreateConnection(); await connection.OpenAsync(ct).ConfigureAwait(false); await using var tx=await connection.BeginTransactionAsync(ct).ConfigureAwait(false);
        var existing=await connection.QuerySingleOrDefaultAsync<(long Id,string RequestHash)>(new CommandDefinition("select id,request_hash as RequestHash from sigov.educacao_rematricula_operacao where tenant_id=@TenantId and entidade_id=@EntidadeId and chave_operacao=@Key for update",new{TenantId=tenantId,EntidadeId=entidadeId,Key=request.ChaveOperacao},tx,cancellationToken:ct)).ConfigureAwait(false);
        if(existing.Id>0){if(existing.RequestHash!=hash)throw new InvalidOperationException("A chave de operação já foi usada com conteúdo diferente."); var prior=await LoadRematriculaResultAsync(connection,tx,tenantId,entidadeId,existing.Id,ct); await tx.CommitAsync(ct).ConfigureAwait(false); return prior;}
        var first=await connection.QuerySingleOrDefaultAsync<(long EscolaId,long Origem,long Destino)>(new CommandDefinition("select m.escola_id as EscolaId,m.ano_letivo_id as Origem,t.ano_letivo_id as Destino from sigov.matricula m join sigov.turma t on t.id=@Turma and t.tenant_id=m.tenant_id and t.entidade_id=m.entidade_id where m.id=@Matricula and m.tenant_id=@TenantId and m.entidade_id=@EntidadeId",new{TenantId=tenantId,EntidadeId=entidadeId,Matricula=request.Itens.First().MatriculaOrigemId,Turma=request.Itens.First().TurmaDestinoId},tx,cancellationToken:ct)).ConfigureAwait(false);
        if(first.EscolaId<=0||first.Origem==first.Destino)throw new InvalidOperationException("Contexto de origem e destino inválido.");
        var opId=await connection.ExecuteScalarAsync<long>(new CommandDefinition(@"insert into sigov.educacao_rematricula_operacao(tenant_id,entidade_id,escola_id,ano_letivo_origem_id,ano_letivo_destino_id,chave_operacao,request_hash,situacao,quantidade_itens,responsavel_id,created_by) values(@TenantId,@EntidadeId,@Escola,@Origem,@Destino,@Key,@Hash,'PROCESSANDO',@Count,@Usuario,@Usuario) returning id",new{TenantId=tenantId,EntidadeId=entidadeId,Escola=first.EscolaId,first.Origem,first.Destino,Key=request.ChaveOperacao,Hash=hash,Count=request.Itens.Count,Usuario=usuarioId},tx,cancellationToken:ct)).ConfigureAwait(false);
        foreach(var item in request.Itens){await tx.SaveAsync("item",ct).ConfigureAwait(false);try{var id=await connection.ExecuteScalarAsync<long>(new CommandDefinition("select sigov.fn_educacao_confirmar_rematricula(@TenantId,@EntidadeId,@ExercicioId,@Operacao,@Origem,@Destino,@Token,@Usuario)",new{TenantId=tenantId,EntidadeId=entidadeId,ExercicioId=exercicioId,Operacao=opId,Origem=item.MatriculaOrigemId,Destino=item.TurmaDestinoId,Token=item.TokenConferencia,Usuario=usuarioId},tx,cancellationToken:ct)).ConfigureAwait(false);await connection.ExecuteAsync(new CommandDefinition("insert into sigov.educacao_rematricula_item(tenant_id,entidade_id,operacao_id,matricula_origem_id,turma_destino_id,matricula_destino_id,token_conferencia,situacao,mensagem,created_by) values(@TenantId,@EntidadeId,@Operacao,@Origem,@Turma,@Matricula,@Token,'CONCLUIDO','Matrícula de destino criada e relida.',@Usuario)",new{TenantId=tenantId,EntidadeId=entidadeId,Operacao=opId,Origem=item.MatriculaOrigemId,Turma=item.TurmaDestinoId,Matricula=id,Token=item.TokenConferencia,Usuario=usuarioId},tx,cancellationToken:ct)).ConfigureAwait(false);}catch(PostgresException ex){await tx.RollbackAsync("item",ct).ConfigureAwait(false);await connection.ExecuteAsync(new CommandDefinition("insert into sigov.educacao_rematricula_item(tenant_id,entidade_id,operacao_id,matricula_origem_id,turma_destino_id,token_conferencia,situacao,mensagem,created_by) values(@TenantId,@EntidadeId,@Operacao,@Origem,@Turma,@Token,'FALHA',@Mensagem,@Usuario)",new{TenantId=tenantId,EntidadeId=entidadeId,Operacao=opId,Origem=item.MatriculaOrigemId,Turma=item.TurmaDestinoId,Token=item.TokenConferencia,Mensagem=ex.MessageText,Usuario=usuarioId},tx,cancellationToken:ct)).ConfigureAwait(false);}}
        await connection.ExecuteAsync(new CommandDefinition("update sigov.educacao_rematricula_operacao o set concluidos=(select count(*) from sigov.educacao_rematricula_item where operacao_id=o.id and situacao='CONCLUIDO'),falhas=(select count(*) from sigov.educacao_rematricula_item where operacao_id=o.id and situacao='FALHA'),situacao=case when not exists(select 1 from sigov.educacao_rematricula_item where operacao_id=o.id and situacao='FALHA') then 'CONCLUIDA' when exists(select 1 from sigov.educacao_rematricula_item where operacao_id=o.id and situacao='CONCLUIDO') then 'PARCIAL' else 'FALHA' end,updated_at=now() where id=@Id",new{Id=opId},tx,cancellationToken:ct)).ConfigureAwait(false);
        var result=await LoadRematriculaResultAsync(connection,tx,tenantId,entidadeId,opId,ct);await tx.CommitAsync(ct).ConfigureAwait(false);return result;
    }

    private static async Task<RematriculaConfirmacaoResponse> LoadRematriculaResultAsync(NpgsqlConnection c,NpgsqlTransaction tx,long tenantId,long entidadeId,long id,CancellationToken ct){var op=await c.QuerySingleAsync<(string Situacao,int Concluidos,int Falhas)>(new CommandDefinition("select situacao as Situacao,concluidos as Concluidos,falhas as Falhas from sigov.educacao_rematricula_operacao where id=@Id and tenant_id=@TenantId and entidade_id=@EntidadeId",new{Id=id,TenantId=tenantId,EntidadeId=entidadeId},tx,cancellationToken:ct));var items=(await c.QueryAsync<RematriculaItemResultado>(new CommandDefinition("select matricula_origem_id as MatriculaOrigemId,matricula_destino_id as MatriculaDestinoId,situacao as Situacao,mensagem as Mensagem from sigov.educacao_rematricula_item where operacao_id=@Id order by id",new{Id=id},tx,cancellationToken:ct))).AsList();return new(id,op.Situacao,op.Concluidos,op.Falhas,items);}
    public async Task<IReadOnlyCollection<RematriculaOperacaoResponse>> ListarOperacoesRematriculaAsync(long tenantId,long entidadeId,CancellationToken ct){using var c=_context.CreateConnection();return(await c.QueryAsync<RematriculaOperacaoResponse>(new CommandDefinition("select id as Id,chave_operacao as ChaveOperacao,escola_id as EscolaId,ano_letivo_origem_id as AnoLetivoOrigemId,ano_letivo_destino_id as AnoLetivoDestinoId,situacao as Situacao,quantidade_itens as QuantidadeItens,concluidos as Concluidos,falhas as Falhas,responsavel_id as ResponsavelId,created_at as ExecutadaEm from sigov.educacao_rematricula_operacao where tenant_id=@TenantId and entidade_id=@EntidadeId order by id desc limit 100",new{TenantId=tenantId,EntidadeId=entidadeId},cancellationToken:ct))).AsList();}
