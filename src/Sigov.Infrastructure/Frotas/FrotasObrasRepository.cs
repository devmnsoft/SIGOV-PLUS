...
public sealed class FrotasObrasRepository : IObrasRepository
{
    ...
    async Task<ObrasDashboardDto> IObrasRepository.DashboardAsync(long t,CancellationToken ct){const string sql=@"select (select count(*) from sigov.obra where tenant_id=@TenantId and status='ATIVA' and not is_deleted) ObrasAtivas,(select count(*) from sigov.obra where tenant_id=@TenantId and status='ATRASADA' and not is_deleted) ObrasAtrasadas,(select count(*) from sigov.obra_medicao where tenant_id=@TenantId and status='PENDENTE' and not is_deleted) MedicoesPendentes,(select count(*) from sigov.obra_fiscalizacao where tenant_id=@TenantId and data_referencia>=date_trunc('month',now()) and not is_deleted) FiscalizacoesMes,(select coalesce(sum(valor),0) from sigov.obra_medicao where tenant_id=@TenantId and status='APROVADA' and not is_deleted) ValorMedido";using var cn=_context.CreateConnection();return await cn.QuerySingleAsync<ObrasDashboardDto>(new CommandDefinition(sql,new{TenantId=t},cancellationToken:ct));}
    private static string Correlation(string value)=>Guid.TryParse(value,out var id)?id.ToString():Guid.NewGuid().ToString();
}
