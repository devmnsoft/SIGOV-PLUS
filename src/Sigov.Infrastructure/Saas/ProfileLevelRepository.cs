...
public sealed class ProfileLevelRepository : IProfileLevelRepository
{
    ...
    {
        const string sql = """
            select codigo as Codigo, nome as Nome, descricao as Descricao, nivel_hierarquico as NivelHierarquico, global as Global, tenant_admin as TenantAdmin, ativo as Ativo
    ...
    {
        const string sql = """
            select distinct coalesce(pn.codigo, pa.codigo_externo, upper(replace(pa.nome, ' ', '_'))) as codigo
    ...
    {
        const string sql = """
            select distinct pe.modulo || '.' || pe.recurso || '.' || pe.acao
            from sigov.usuario u
    ...
    {
        const string sql = """
            select tenant_id as TenantId, entidade_id as EntidadeId, exercicio_id as ExercicioId, modulo_codigo as ModuloCodigo, escopo as Escopo
