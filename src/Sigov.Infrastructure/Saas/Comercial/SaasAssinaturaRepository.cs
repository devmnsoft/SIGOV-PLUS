...
public sealed class SaasAssinaturaRepository : ISaasAssinaturaRepository
{
    ...
    {
        const string sql = @"select a.id as Id, a.tenant_id as TenantId, a.plano_id as PlanoId, p.codigo as PlanoCodigo, p.nome as PlanoNome, a.status as Status,
...
";
    ...
    {
        var sql = $@"select a.id as Id, a.tenant_id as TenantId, a.plano_id as PlanoId, p.codigo as PlanoCodigo, p.nome as PlanoNome, a.status as Status,
       a.data_inicio as DataInicio, a.data_fim as DataFim, a.usuarios_contratados as UsuariosContratados, a.entidades_contratadas as EntidadesContratadas,
       a.valor_contratado as ValorContratado, a.periodicidade as Periodicidade,
       coalesce(array_agg(am.modulo_codigo order by am.modulo_codigo) filter (where am.modulo_codigo is not null), array[]::varchar[]) as Modulos
from sigov.saas_assinatura a join sigov.saas_plano p on p.id=a.plano_id
...
";
    ...
    {
        const string sql = @"select l.codigo as Codigo, l.nome as Nome, l.valor as Valor, l.unidade as Unidade, l.ilimitado as Ilimitado
...
";
    ...
    {
        const string sql = @"update sigov.saas_assinatura set status=@Status, data_fim=@DataFim, usuarios_contratados=@UsuariosContratados, entidades_contratadas=@EntidadesContratadas,
