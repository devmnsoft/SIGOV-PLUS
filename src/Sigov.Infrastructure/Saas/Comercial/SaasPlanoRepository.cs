...
public sealed class SaasPlanoRepository : ISaasPlanoRepository
{
    ...
    {
        var sql = $"""
            select p.id as Id, p.codigo as Codigo, p.nome as Nome, p.descricao as Descricao, p.publico as Publico, p.destaque as Destaque, p.ordem as Ordem,
    ...
    {
        const string sql = """
            update sigov.saas_plano set nome=@Nome, descricao=@Descricao, publico=@Publico, destaque=@Destaque, ordem=@Ordem, preco_base=@PrecoBase,
