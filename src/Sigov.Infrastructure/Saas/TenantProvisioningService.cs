    ...
    public async Task<ProvisionTenantResult> ProvisionarAsync(ProvisionTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var tenantId = await connection.ExecuteScalarAsync<long>(new CommandDefinition("""
                insert into sigov.tenant (nome, documento, slug, status, ambiente, data_inicio_operacao)
            ...
            if (!string.IsNullOrWhiteSpace(request.Dominio))
            {
                await connection.ExecuteAsync(new CommandDefinition("""
                    insert into sigov.tenant_dominio (tenant_id, tipo, dominio, principal, verificado)
            ...
            var planoId = await connection.ExecuteScalarAsync<long>(new CommandDefinition("""
                insert into sigov.plano_saas (codigo, nome, ativo)
            ...
            await connection.ExecuteAsync(new CommandDefinition("""
                insert into sigov.tenant_assinatura (tenant_id, plano_saas_id, status, inicio_at, ativo)
            ...
            {
                var moduloId = await connection.ExecuteScalarAsync<long>(new CommandDefinition("""
                    insert into sigov.modulo_saas (codigo, nome, categoria, ativo)
            ...
            var entidadeId = await connection.ExecuteScalarAsync<long?>(new CommandDefinition("""
                insert into sigov.entidade (tenant_id, nome, cnpj, ativo)
            ...
            if (entidadeId.HasValue)
            {
                ...
                var pessoaId = await connection.ExecuteScalarAsync<long>(new CommandDefinition("""
                    insert into sigov.pessoa (tenant_id, entidade_id, exercicio_id, tipo_pessoa, nome, documento, ativo)
                ...
                usuarioId = await connection.ExecuteScalarAsync<long>(new CommandDefinition("""
                    insert into sigov.usuario (tenant_id, entidade_id, exercicio_id, pessoa_id, login, email, senha_hash, tipo_usuario, ativo)
            ...
            await connection.ExecuteAsync(new CommandDefinition("""
                insert into sigov.tenant_evento_operacional (tenant_id, tipo, severidade, mensagem, payload)
