...
select distinct p.chave from sigov.usuario_grupo ug
 ...
 where ug.usuario_id=@UserId and not ug.is_deleted;";
    ...
    {
        const string sql = @"select u.id, u.tenant_id as TenantId from sigov.usuario u left join sigov.tenant t on t.id=u.tenant_id
...
and (lower(u.login)=lower(@Value) or lower(u.email)=lower(@Value)) limit 1;";
    ...
    public async Task StorePasswordResetTokenAsync(AccountReference account, string tokenHash, Guid correlationId, CancellationToken cancellationToken)
    {
        const string sql = @"insert into sigov.senha_redefinicao_token(tenant_id, usuario_id, token_hash, expira_at, correlation_id)
values(@TenantId,@UsuarioId,@TokenHash,now()+interval '30 minutes',@CorrelationId);";
        using var connection = connectionFactory.CreateConnection();
        await connection.ExecuteAsync(new CommandDefinition(sql, new { account.TenantId, UsuarioId = account.Id, TokenHash = tokenHash, CorrelationId = correlationId }, cancellationToken: cancellationToken)).ConfigureAwait(false);
    }
 ...
 returning rt.usuario_id, rt.tenant_id)
