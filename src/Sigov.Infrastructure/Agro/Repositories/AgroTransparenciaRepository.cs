...
public sealed class AgroTransparenciaRepository : IAgroTransparenciaRepository
{
    ...
    {
        const string sql = """
            insert into sigov.agro_dataset_publico(tenant_id,entidade_id,codigo,nome,descricao,tipo_dataset,formato_padrao,anonimizado,publico,created_by)
    ...
    public async Task<AgroDatasetPublicacaoResponse> PublicarAsync(long tenantId, long datasetId, long usuarioId, PublicarAgroDatasetRequest request, CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            insert into sigov.agro_dataset_publicacao(tenant_id,dataset_id,status,formato,conteudo_texto,hash_sha256,total_registros,publicado_at,publicado_by)
    ...
    {
        const string sql = """
            select p.id as Id, p.tenant_id as TenantId, p.dataset_id as DatasetId, p.status as Status, p.formato as Formato, p.conteudo_texto as ConteudoTexto, p.total_registros as TotalRegistros, p.publicado_at as PublicadoAt
