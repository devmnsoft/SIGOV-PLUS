(() => {
  ...
  const toast = (msg, isError = false) => { const el = root.querySelector('.enterprise-toast'); el.querySelector('.toast-body').textContent = msg; el.classList.toggle('text-bg-danger', isError); bootstrap.Toast.getOrCreateInstance(el).show(); };
  ...
  async function load() {
    try {
      ...
      body.innerHTML = items.map(item => {
        ...
        return `<tr data-id="${id}" data-row='${JSON.stringify(item).replace(/'/g, '&apos;')}'><td>${item.name || item.Name || ''}</td><td><span class="badge bg-secondary">${item.status || item.Status || ''}</span></td><td>${item.documentMasked || item.DocumentMasked || '-'}</td><td>${item.emailMasked || item.EmailMasked || '-'}</td><td>${item.phoneMasked || item.PhoneMasked || '-'}</td><td>${item.updatedAt || item.UpdatedAt || ''}</td><td class="text-end"><button class="btn btn-sm btn-outline-primary enterprise-details">Detalhes</button> <button class="btn btn-sm btn-outline-secondary enterprise-edit">Editar</button> <button class="btn btn-sm btn-outline-danger enterprise-delete">Inativar</button></td></tr>`;
      }).join('');
  ...
  form?.addEventListener('submit', async ev => {
    ...
    const id = data.id; delete data.id; const method = id ? 'PUT' : 'POST'; const url = id ? `${actionApi}/${id}` : actionApi; const r = await fetch(url, { method, headers: headers(), body: JSON.stringify(data) });
    toast(r.ok ? 'Registro salvo com auditoria.' : 'Falha ao salvar registro.', !r.ok);
  ...
  root.addEventListener('click', ev => {
    ...
    if (ev.target.matches('.enterprise-edit')) { form.elements.id.value = tr.dataset.id; form.elements.nome.value = tr.children[0].innerText; form.elements.status.value = tr.children[1].innerText; bootstrap.Modal.getOrCreateInstance('#enterpriseFormModal').show(); }
    if (ev.target.matches('.enterprise-delete')) { if (!confirm('Confirmar inativação deste registro?')) return; fetch(`${actionApi}/${tr.dataset.id}`, { method: 'DELETE', headers: headers() }).then(async r => { toast(r.ok ? 'Registro inativado com auditoria.' : 'Falha ao inativar registro.', !r.ok); if (r.ok) await load(); }); }
  });
