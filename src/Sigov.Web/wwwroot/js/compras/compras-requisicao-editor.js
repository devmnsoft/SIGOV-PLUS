...
form?.addEventListener('submit',async event=>{
  ...
  lock(button,true);
  try{const result=await request(editing?`/api/compras-empresariais/requisicoes/${form.dataset.id}`:'/api/compras-empresariais/requisicoes',{method:editing?'PUT':'POST',body:JSON.stringify(payload)});dirty=false;window.location.assign(`/ComprasEmpresariais/Requisicoes/${editing?form.dataset.id:result.id}`);}
