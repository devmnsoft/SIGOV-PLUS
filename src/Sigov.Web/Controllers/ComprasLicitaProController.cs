...
public sealed class ComprasLicitaProController(ILicitaProService service,ICurrentTenant tenant,ICurrentUser user,IAuthorizationEvaluator authorization) : Controller
{
    ...
    public async Task<IActionResult> Documento(DocumentoFornecedorInput input,CancellationToken ct)=>await Post(input,LicitaProPermissoes.DocumentoGerir,async()=>await service.CriarDocumentoAsync(T(),E(),U(),Trace(),input,ct),"Documentos",ct);
    [HttpPost("/Compras/LicitaPro/Agenda/Nova"),ValidateAntiForgeryToken]
    public async Task<IActionResult> Agenda(AgendaPropostaInput input,CancellationToken ct)=>await Post(input,LicitaProPermissoes.AgendaGerir,async()=>await service.CriarAgendaAsync(T(),E(),U(),Trace(),input,ct),"Agenda",ct);
