...
public sealed class PatrimonioController(IPatrimonioService service,ICurrentTenant tenant,ICurrentUser user,IAuthorizationEvaluator authorization) : Controller
{
    ...
    public async Task<IActionResult> Detalhe(long id,CancellationToken ct){if(!await Permitido(PatrimonioPermissoes.BemVisualizar,ct))return Forbid();var detalhe=await service.ObterBemDetalheAsync(Tenant(),id,ct);return detalhe is null?NotFound():View(detalhe);}
