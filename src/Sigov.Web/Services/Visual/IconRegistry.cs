...
public sealed class IconRegistry : IIconRegistry
{
    ...
    private static IReadOnlyDictionary<string, IconDefinition> Build()
    {
        ...
        return navigation.Select(x => new IconDefinition(x, x, "navigation"))
            .Concat(actions.Select(x => new IconDefinition(x, x, "action")))
            .Concat(states.Select(x => new IconDefinition(x, x, "state")))
