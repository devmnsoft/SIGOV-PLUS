...
public sealed class IndustriaController : ControllerBase
{
    ...
    private async Task<ActionResult<ApiResponse<object>>> ApontarAsync(long id, ApontamentoRequest r)
    {
        ...
        try
        {
            ...
            var quantidade_produzida = ordem.QuantidadeProduzida;
            if (r.Tipo == "PRODUCAO" && quantidade_produzida + r.QuantidadeBoas > ordem.QuantidadePlanejada) return Conflict(ApiResponse<object>.Fail("A produção acumulada ultrapassa a quantidade planejada; nenhuma tolerância está configurada.", cid));
