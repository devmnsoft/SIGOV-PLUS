...
public sealed class AlmoxarifadoController(IAlmoxarifadoService service,ICurrentTenant tenant,ICurrentUser user,IAuthorizationEvaluator auth):ControllerBase
{
 ...
 [HttpGet("transferencias")]public Task<IActionResult> Transferencias(string? status,int pagina=1,CancellationToken ct=default)=>Run(AlmoxarifadoPermissoes.TransferenciaVisualizar,()=>service.ListarTransferenciasAsync(T(),E(),status,pagina,ct),ct);
 [HttpGet("transferencias/{id:long}")]public Task<IActionResult> Transferencia(long id,CancellationToken ct)=>Run(AlmoxarifadoPermissoes.TransferenciaVisualizar,async()=>await service.ObterTransferenciaAsync(T(),E(),id,ct)??throw new KeyNotFoundException("Transferência não encontrada."),ct);
 [HttpPost("transferencias")]public Task<IActionResult> CriarTransferencia(TransferenciaInput input,CancellationToken ct)=>Run(AlmoxarifadoPermissoes.TransferenciaCriar,()=>service.CriarTransferenciaAsync(T(),E(),tenant.ExercicioId,U(),Trace(),input,ct),ct);
 [HttpPost("transferencias/{id:long}/expedicao")]public Task<IActionResult> ExpedirTransferencia(long id,CancellationToken ct)=>Run(AlmoxarifadoPermissoes.TransferenciaExpedir,async()=>{await service.ExpedirTransferenciaAsync(T(),E(),U(),Trace(),id,ct);return new{id,status="EM_TRANSITO"};},ct);
 [HttpPost("transferencias/{id:long}/recebimentos")]public Task<IActionResult> ReceberTransferencia(long id,TransferenciaRecebimentoInput input,CancellationToken ct)=>Run(AlmoxarifadoPermissoes.TransferenciaReceber,async()=>{await service.ReceberTransferenciaAsync(T(),E(),U(),Trace(),id,input,ct);return new{id,registrado=true};},ct);
