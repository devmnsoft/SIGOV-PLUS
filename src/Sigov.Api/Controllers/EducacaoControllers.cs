...
public sealed class MatriculasController : EducacaoApiControllerBase
{
    ...
    [HttpPost("{id:long}/enturmar")] public async Task<ActionResult<ApiResponse<object>>> Enturmar(long id, [FromBody] EnturmarMatriculaRequest request, CancellationToken ct) => FromResult(await _service.EnturmarAsync(id, request, ct).ConfigureAwait(false));
...
public sealed class NotaLancamentoController : EducacaoApiControllerBase
{
    ...
    [HttpPost]
    [HttpPost("/api/educacao/avaliacoes/notas")]
    public async Task<ActionResult<ApiResponse<long>>> Lancar([FromBody] EducacaoNotaLancamentoRequest request, CancellationToken ct) => FromResult(await _service.RegistrarNotaAsync(request.AvaliacaoId, new NotaCreateRequest(request.AlunoId, request.Valor, request.Observacao), ct).ConfigureAwait(false));
}
...
public sealed class PreMatriculasController : EducacaoApiControllerBase
{
    private readonly IPreMatriculaService _service;
    public PreMatriculasController(IPreMatriculaService service) => _service = service;
    [HttpGet] public async Task<ActionResult<ApiResponse<PagedResult<PreMatriculaResponse>>>> Listar([FromQuery] PreMatriculaFiltro filtro, CancellationToken ct) => FromResult(await _service.ListarAsync(filtro, ct).ConfigureAwait(false));
    [HttpPost] public async Task<ActionResult<ApiResponse<long>>> Criar([FromBody] PreMatriculaCreateRequest request, CancellationToken ct) => FromResult(await _service.CriarAsync(request, ct).ConfigureAwait(false));
    [HttpPut("{id:long}/rascunho")] public async Task<ActionResult<ApiResponse<object>>> Editar(long id, [FromBody] PreMatriculaEdicaoRequest request, CancellationToken ct) => FromResult(await _service.EditarRascunhoAsync(id, request, ct).ConfigureAwait(false));
    [HttpPost("{id:long}/transicoes/{destino}")] public async Task<ActionResult<ApiResponse<object>>> Transicionar(long id, string destino, [FromBody] PreMatriculaTransicaoRequest request, CancellationToken ct) => FromResult(await _service.TransicionarAsync(id, destino, request, ct).ConfigureAwait(false));
    [HttpPost("{id:long}/converter-matricula")] public async Task<ActionResult<ApiResponse<object>>> Converter(long id, [FromBody] ConverterPreMatriculaRequest request, CancellationToken ct) => FromResult(await _service.ConverterAsync(id, request, ct).ConfigureAwait(false));
    [HttpPost("{id:long}/indeferir")] public async Task<ActionResult<ApiResponse<object>>> Indeferir(long id, [FromBody] PreMatriculaTransicaoRequest request, CancellationToken ct) => FromResult(await _service.IndeferirAsync(id, request, ct).ConfigureAwait(false));
    [HttpPost("ofertas")] public async Task<ActionResult<ApiResponse<long>>> Ofertar([FromBody] OfertaVagaRequest request, CancellationToken ct) => FromResult(await _service.CriarOfertaAsync(request, ct).ConfigureAwait(false));
    [HttpPost("ofertas/{id:long}/decisao")] public async Task<ActionResult<ApiResponse<object>>> DecidirOferta(long id, [FromBody] OfertaVagaDecisaoRequest request, CancellationToken ct) => FromResult(await _service.DecidirOfertaAsync(id, request, ct).ConfigureAwait(false));
}
