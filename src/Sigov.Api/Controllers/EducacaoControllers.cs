...
public sealed class PreMatriculasController : EducacaoApiControllerBase { private readonly IPreMatriculaService _service; public PreMatriculasController(IPreMatriculaService service)=>_service=service; [HttpGet] public async Task<ActionResult<ApiResponse<PagedResult<PreMatriculaResponse>>>> Listar([FromQuery] PreMatriculaFiltro filtro,CancellationToken ct)=>FromResult(await _service.ListarAsync(filtro,ct).ConfigureAwait(false)); [HttpPost] public async Task<ActionResult<ApiResponse<long>>> Criar([FromBody] PreMatriculaCreateRequest request,CancellationToken ct)=>FromResult(await _service.CriarAsync(request,ct).ConfigureAwait(false)); [HttpPost("{id:long}/converter-matricula")] [HttpPost("{id:long}/deferir")] public async Task<ActionResult<ApiResponse<object>>> Converter(long id,[FromBody] ConverterPreMatriculaRequest request,CancellationToken ct)=>FromResult(await _service.ConverterAsync(id,request,ct).ConfigureAwait(false)); [HttpPost("{id:long}/indeferir")] public async Task<ActionResult<ApiResponse<object>>> Indeferir(long id,CancellationToken ct)=>FromResult(await _service.IndeferirAsync(id,ct).ConfigureAwait(false)); }
