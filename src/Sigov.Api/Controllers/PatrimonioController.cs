...
public sealed class PatrimonioController(IPatrimonioService service, ICurrentTenant tenant, ICurrentUser user, IAuthorizationEvaluator authorization) : ControllerBase
{
    ...
    [HttpGet("bens/{id:long}")] public async Task<IActionResult> Bem(long id,CancellationToken ct)=>await Executar(PatrimonioPermissoes.BemVisualizar,async()=>await service.ObterBemDetalheAsync(Tenant(),id,ct)??throw new KeyNotFoundException("Bem não encontrado."),ct);
    ...
    [HttpPost("bens/{id:long}/movimentar")] public async Task<IActionResult> Movimentar(long id,PatrimonioMovimentacaoInput input,CancellationToken ct)=>await Executar(PatrimonioPermissoes.BemMovimentar,async()=>{await service.MovimentarBemAsync(Tenant(),Usuario(),IdempotencyKey(),id,input,ct);return new{id};},ct);
    ...
    private string IdempotencyKey()=>Request.Headers.TryGetValue("Idempotency-Key",out var value)&&!string.IsNullOrWhiteSpace(value)?value.ToString():HttpContext.TraceIdentifier;
