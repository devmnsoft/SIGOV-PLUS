...
public sealed class ProcessosDigitaisController : ProcessosControllerBase
{
    ...
    [HttpPost("{id:long}/encerrar")] public async Task<ActionResult<ApiResponse<object>>> Encerrar(long id, [FromBody] EncerrarProcessoRequest request, CancellationToken ct) => FromResult(await _service.EncerrarAsync(id, request, ct).ConfigureAwait(false));
