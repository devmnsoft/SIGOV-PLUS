...
public sealed class OrdensServicoController(IOrdemServicoApplicationService app) : ControllerBase
{
    ...
    [HttpGet("tecnicos"),Authorize(Policy="os.ordens.agendar")] public async Task<IActionResult> Technicians(CancellationToken ct)=>Ok(await app.TecnicosAsync(Contexto(),ct));
    ...
    [HttpGet("{id:guid}/evidencias"),Authorize(Policy="os.evidencias.visualizar")] public async Task<IActionResult> Evidences(Guid id,CancellationToken ct)=>Ok(await app.EvidenciasAsync(Contexto(),id,ct));
    [HttpPost("{id:guid}/evidencias"),Authorize(Policy="os.evidencias.adicionar")] public async Task<IActionResult> Evidence(Guid id,AdicionarEvidenciaRequest r,CancellationToken ct){await app.AdicionarEvidenciaAsync(Contexto(),id,r,Key(),ct);return NoContent();}
    [HttpGet("{id:guid}/aceite"),Authorize(Policy="os.aceite.visualizar")] public async Task<IActionResult> Acceptance(Guid id,CancellationToken ct){var aceite=await app.ObterAceiteAsync(Contexto(),id,ct);return aceite is null?NotFound():Ok(aceite);}
    [HttpPost("{id:guid}/aceite"),Authorize(Policy="os.aceite.registrar")] public async Task<IActionResult> Accept(Guid id,RegistrarAceiteClienteRequest r,CancellationToken ct){await app.RegistrarAceiteAsync(Contexto(),id,r,Key(),ct);return NoContent();}
    [HttpGet("{id:guid}/custos"),Authorize(Policy="os.custos.visualizar")] public async Task<IActionResult> Costs(Guid id,CancellationToken ct)=>Ok(await app.ObterCustosAsync(Contexto(),id,ct));
