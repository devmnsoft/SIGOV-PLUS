...
public sealed class EducacaoDiarioClasseController : EducacaoApiControllerBase
{
    ...
    [HttpGet("{id:long}/conferencia")] public async Task<ActionResult<ApiResponse<EducacaoDiarioConferenciaDto>>> Conferir(long id,CancellationToken ct)=>FromResult(await _service.ConferirDiarioAsync(id,ct));
    [HttpPost("{id:long}/fechar-periodo")] public async Task<ActionResult<ApiResponse<long>>> Fechar(long id,EducacaoDiarioFechamentoRequest r,CancellationToken ct)=>FromResult(await _service.FecharDiarioAsync(id,r,ct));
    [HttpPost("{id:long}/reabrir")] public async Task<ActionResult<ApiResponse<object>>> Reabrir(long id,EducacaoDiarioReaberturaRequest r,CancellationToken ct)=>FromResult(await _service.ReabrirDiarioAsync(id,r,ct));
    [HttpGet("{id:long}/historico-fechamentos")] public async Task<ActionResult<ApiResponse<IReadOnlyCollection<EducacaoDiarioFechamentoDto>>>> Historico(long id,CancellationToken ct)=>FromResult(await _service.HistoricoFechamentoAsync(id,ct));
...
public sealed class EducacaoPortalController : EducacaoApiControllerBase
{
    ...
    [HttpGet("alunos")] public async Task<ActionResult<ApiResponse<IReadOnlyCollection<EducacaoPortalAlunoDto>>>> Alunos(CancellationToken ct)=>FromResult(await _service.ListarAlunosAsync(ct));
    [HttpGet("alunos/{alunoId:long}/vida-escolar")] public async Task<ActionResult<ApiResponse<EducacaoPortalVidaEscolarDto>>> VidaEscolar(long alunoId,CancellationToken ct)=>FromResult(await _service.ObterVidaEscolarAsync(alunoId,ct));
    [HttpGet("alunos/{alunoId:long}/boletim")] public async Task<ActionResult<ApiResponse<EducacaoPortalBoletimDto>>> Boletim(long alunoId,CancellationToken ct)=>FromResult(await _service.ObterBoletimAsync(alunoId,ct));
    ...
    [HttpPost("alunos/{alunoId:long}/comunicados/{comunicadoId:long}/ciencia")] public async Task<ActionResult<ApiResponse<EducacaoPortalCienciaDto>>> Ciencia(long alunoId,long comunicadoId,CancellationToken ct)=>FromResult(await _service.ConfirmarCienciaAsync(comunicadoId,alunoId,ct));
    [HttpPost("alunos/{alunoId:long}/comunicados/{comunicadoId:long}/leitura")] public async Task<ActionResult<ApiResponse<object>>> Leitura(long alunoId,long comunicadoId,CancellationToken ct)=>FromResult(await _service.RegistrarLeituraAsync(comunicadoId,alunoId,ct));
    ...
    [HttpPost("admin/vinculos/{id:long}/ativar")] public async Task<ActionResult<ApiResponse<object>>> AtivarVinculo(long id,EducacaoPortalAlterarVinculoRequest r,CancellationToken ct)=>FromResult(await _service.AlterarVinculoAsync(id,true,r.Justificativa,ct));
    [HttpPost("admin/vinculos/{id:long}/revogar")] public async Task<ActionResult<ApiResponse<object>>> RevogarVinculo(long id,EducacaoPortalAlterarVinculoRequest r,CancellationToken ct)=>FromResult(await _service.AlterarVinculoAsync(id,false,r.Justificativa,ct));
